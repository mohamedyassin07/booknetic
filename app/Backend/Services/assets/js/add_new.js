	...
	$(document).ready(function()
	{
		...
		}).on('click', '#addServiceSave', function ()
		{
			var name					= $(".fs-modal #input_name").val(),
				...
				fix_time				= $(".fs-modal #is_fix_time_checkbox").is(':checked') ? 1 : 0,

				capacity				= $(".fs-modal #select_capacity").val(),
				min_capacity			= capacity === '0' ? 1 : $(".fs-modal #input_min_capacity").val(),
				max_capacity			= capacity === '0' ? 1 : $(".fs-modal #input_max_capacity").val(),

				employees				= [],
				activate_zoom           = $('.fs-modal #activate_zoom').is(':checked') ? 1 : 0,
			...
			data.append('fix_time', fix_time);

			data.append('min_capacity', min_capacity);
			data.append('max_capacity', max_capacity);

			data.append('employees', JSON.stringify( employees ));
			data.append('activate_zoom', activate_zoom);

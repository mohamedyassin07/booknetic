 ...
 */
class Codabar extends \Booknetic_Mpdf\Barcode\AbstractBarcode implements \Booknetic_Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio)
	{
		...
		for ($i = 0; $i < $len; ++$i) {
			if (!isset($chr[$code[$i]])) {
				throw new \Booknetic_Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" CODABAR barcode value', $code[$i]));
			}
			$seq = $chr[$code[$i]];
			for ($j = 0; $j < 8; ++$j) {
				if (($j % 2) == 0) {

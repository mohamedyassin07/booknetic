...
namespace Booknetic_Mpdf\Barcode;
 ...
 */
class Code128 extends \Booknetic_Mpdf\Barcode\AbstractBarcode implements \Booknetic_Mpdf\Barcode\BarcodeInterface
{
		...
		switch (strtoupper($type)) {
			case 'RAW':
				...
				foreach (explode(" ", $code) as $v) {
					if (is_numeric($v) && round($v, 0) == $v) {
						if ($v>=0 && $v<=105) {
							...
							throw new \Booknetic_Mpdf\Barcode\BarcodeException('Invalid CODE128RAW barcode value. 0-105 needed');
						}
					} else {
				...
				if ($startid<103 || $startid>105) {
					throw new \Booknetic_Mpdf\Barcode\BarcodeException('Invalid CODE128RAW startid value. Must 103,104 or 105 (for A,B or C)');
				}
				$keys='';
				for ($i = 0; $i <= 105; ++$i) {
					$keys .= chr($i);
			...
			case 'C':
				...
				if ((strlen($code) % 2) != 0) {
					...
					throw new \Booknetic_Mpdf\Barcode\BarcodeException('Invalid CODE128C barcode value');
				}
				for ($i = 0; $i <= 99; ++$i) {
					$keys .= chr($i);
				}
				...
				for ($i = 0; $i < $hclen; ++$i) {
					if ($code[2 * $i]<"0" || $code[2 * $i]>"9" || $code[2 * $i + 1]<"0" || $code[2 * $i + 1]>"9") {
						throw new \Booknetic_Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE128C barcode value', $code[$i]));
					}
				...
				break;
			default:
				throw new \Booknetic_Mpdf\Barcode\BarcodeException('Invalid CODE128 barcode type');
		}
		...
		for ($i = 0; $i < $clen; ++$i) {
			if ($ean && $i == 0) {
				...
				if (strpos($keys, $code[$i]) === false) {
					throw new \Booknetic_Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE128'.$type.' barcode value', $code[$i]));
				}

 ...
 */
class Code93 extends \Booknetic_Mpdf\Barcode\AbstractBarcode implements \Booknetic_Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			if (ord($code[$i]) > 127) {
				throw new \Booknetic_Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in Code93 barcode value', $code[$i]));
			}
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$char])) {
				// invalid character
				throw new \Booknetic_Mpdf\Barcode\BarcodeException('Invalid CODE93 barcode value');
			}
			for ($j = 0; $j < 6; ++$j) {
				if (($j % 2) == 0) {
					$t = true; // bar

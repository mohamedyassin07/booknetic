 ...
 */
class S25 extends \Booknetic_Mpdf\Barcode\AbstractBarcode implements \Booknetic_Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $checksum)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$digit])) {
				// invalid character
				throw new \Booknetic_Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in S25 barcode value', $digit));
			}
			$seq .= $chr[$digit];
		}

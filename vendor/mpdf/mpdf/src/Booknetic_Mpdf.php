...
use Booknetic_Mpdf\Config\ConfigVariables;
use Booknetic_Mpdf\Config\FontVariables;

use Booknetic_Mpdf\Conversion;

use Booknetic_Mpdf\Css\Border;
use Booknetic_Mpdf\Css\TextVars;

use Booknetic_Mpdf\Log\Context as LogContext;

use Booknetic_Mpdf\Fonts\MetricsGenerator;

use Booknetic_Mpdf\Output\Destination;

use Booknetic_Mpdf\QrCode;

use Booknetic_Mpdf\Utils\Arrays;
use Booknetic_Mpdf\Utils\NumericString;
use Booknetic_Mpdf\Utils\UtfString;
 ...
 * @license GPL-2.0
 */
class Booknetic_Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \Booknetic_Mpdf\Gradient
	 */
	private $gradient;

	/**
	 * @var \Booknetic_Mpdf\Image\Bmp
	 */
	private $bmp;

	/**
	 * @var \Booknetic_Mpdf\Image\Wmf
	 */
	private $wmf;
	...
	/**
	 * @var \Booknetic_Mpdf\DirectWrite
	 */
	private $directWrite;
	...
	/**
	 * @var \Booknetic_Mpdf\Fonts\FontCache
	 */
	private $fontCache;

	/**
	 * @var \Booknetic_Mpdf\Fonts\FontFileFinder
	 */
	private $fontFileFinder;

	/**
	 * @var \Booknetic_Mpdf\Tag
	 */
	private $tag;

	/**
	 * @var \Booknetic_Mpdf\Barcode
	 * @todo solve Tag dependency and make private
	 */
	public $barcode;

	/**
	 * @var \Booknetic_Mpdf\QrCode\QrCode
	 */
	private $qrcode;
	...
	/**
	 * @var \Booknetic_Mpdf\Color\ColorModeConverter
	 */
	private $colorModeConverter;

	/**
	 * @var \Booknetic_Mpdf\Color\ColorSpaceRestrictor
	 */
	private $colorSpaceRestrictor;

	/**
	 * @var \Booknetic_Mpdf\Hyphenator
	 */
	private $hyphenator;

	/**
	 * @var \Booknetic_Mpdf\Pdf\Protection
	 */
	private $protection;

	/**
	 * @var \Booknetic_Mpdf\RemoteContentFetcher
	 */
	...
	/**
	 * @var \Booknetic_Mpdf\Language\ScriptToLanguageInterface
	 */
	...
	/**
	 * @var \Booknetic_Mpdf\Writer\FontWriter
	 */
	private $fontWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\MetadataWriter
	 */
	private $metadataWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\ImageWriter
	 */
	private $imageWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\FormWriter
	 */
	private $formWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\PageWriter
	 */
	private $pageWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\BookmarkWriter
	 */
	private $bookmarkWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\OptionalContentWriter
	 */
	private $optionalContentWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\ColorWriter
	 */
	private $colorWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\BackgroundWriter
	 */
	private $backgroundWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\JavaScriptWriter
	 */
	private $javaScriptWriter;

	/**
	 * @var \Booknetic_Mpdf\Writer\ResourceWriter
	 */
	...
	public function __construct(array $config = [])
	{
		...
		// Line width (0.2 mm)
		$this->LineWidth = .567 / Booknetic_Mpdf::SCALE;
		...
		if (file_exists($this->defaultCssFile)) {
			...
			throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('Unable to read default CSS file "%s"', $this->defaultCssFile));
		}
		...
		if (!$default_font_size) {
			...
			$default_font_size = $mmsize * (Booknetic_Mpdf::SCALE);
		}
	 ...
	 *
	 * @return \Booknetic_Mpdf\Booknetic_Mpdf
	 */
	...
	function _setPageSize($format, &$orientation)
	{
		if (is_string($format)) {
			...
			if (!$format[0] || !$format[1]) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException('Invalid page format: ' . $format[0] . ' ' . $format[1]);
			}

			$this->fwPt = $format[0] * Booknetic_Mpdf::SCALE;
			$this->fhPt = $format[1] * Booknetic_Mpdf::SCALE;
		}

		$this->fw = $this->fwPt / Booknetic_Mpdf::SCALE;
		$this->fh = $this->fhPt / Booknetic_Mpdf::SCALE;
		...
		if ($orientation === 'p' || $orientation == 'portrait') {
			...
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Incorrect orientation: ' . $orientation);
		}

		$this->CurOrientation = $orientation;

		$this->w = $this->wPt / Booknetic_Mpdf::SCALE;
		$this->h = $this->hPt / Booknetic_Mpdf::SCALE;
	}
	...
	function SetDisplayMode($zoom, $layout = 'continuous')
	{
		...
		if (in_array($zoom, $allowedZoomModes, true) || is_numeric($zoom)) {
			...
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Incorrect zoom display mode: ' . $zoom);
		}
		...
		if (in_array($layout, $allowedLayoutModes, true)) {
			...
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Incorrect layout display mode: ' . $layout);
		}
	}
	...
	function SetVisibility($v)
	{
		...
		if ($v == 'printonly') {
			...
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Incorrect visibility: ' . $v);
		}
		$this->visibility = $v;
	}
	...
	function _resizeBackgroundImage($imw, $imh, $cw, $ch, $resize, $repx, $repy, $pba = [], $size = [])
	{
		...
		$cw = $cw * Booknetic_Mpdf::SCALE;
		$ch = $ch * Booknetic_Mpdf::SCALE;
	...
	function SetBackground(&$properties, &$maxwidth)
	{
		...
		if (preg_match('/(-moz-)*(repeating-)*(linear|radial)-gradient/', $properties['BACKGROUND-IMAGE'])) {
			...
			if (isset($sizesarray['IMAGE_ID'])) {
				...
				$orig_w = $sizesarray['WIDTH'] * Booknetic_Mpdf::SCALE;  // in user units i.e. mm
				$orig_h = $sizesarray['HEIGHT'] * Booknetic_Mpdf::SCALE;  // (using $this->img_dpi)
	...
	function PrintBodyBackgrounds()
	{
		...
		if ($this->bodyBackgroundColor) {
			...
			$s .= sprintf('%.3F %.3F %.3F %.3F re f Q', ($clx * Booknetic_Mpdf::SCALE), ($cly * Booknetic_Mpdf::SCALE), $clw * Booknetic_Mpdf::SCALE, $clh * Booknetic_Mpdf::SCALE) . "\n";
		}
		...
		if ($this->bodyBackgroundImage) {
			if (isset($this->bodyBackgroundImage['gradient']) && $this->bodyBackgroundImage['gradient'] && preg_match('/(-moz-)*(repeating-)*(linear|radial)-gradient/', $this->bodyBackgroundImage['gradient'])) {
				...
				$s .= sprintf('q /Pattern cs /P%d scn %s %.3F %.3F %.3F %.3F re f Q', $n, $opac, ($clx * Booknetic_Mpdf::SCALE), ($cly * Booknetic_Mpdf::SCALE), $clw * Booknetic_Mpdf::SCALE, $clh * Booknetic_Mpdf::SCALE) . "\n";
			}
		}
		/* -- END BACKGROUNDS -- */
		return $s;
	...
	function _setClippingPath($clx, $cly, $clw, $clh)
	{
		...
		$s .= sprintf('%.3F %.3F m ', ($clx) * Booknetic_Mpdf::SCALE, ($this->h - ($cly)) * Booknetic_Mpdf::SCALE); // start point TL before the arc
		$s .= sprintf('%.3F %.3F l ', ($clx) * Booknetic_Mpdf::SCALE, ($this->h - ($cly + $clh)) * Booknetic_Mpdf::SCALE); // line to BL
		$s .= sprintf('%.3F %.3F l ', ($clx + $clw) * Booknetic_Mpdf::SCALE, ($this->h - ($cly + $clh)) * Booknetic_Mpdf::SCALE); // line to BR
		$s .= sprintf('%.3F %.3F l ', ($clx + $clw) * Booknetic_Mpdf::SCALE, ($this->h - ($cly)) * Booknetic_Mpdf::SCALE); // line to TR
		$s .= sprintf('%.3F %.3F l ', ($clx) * Booknetic_Mpdf::SCALE, ($this->h - ($cly)) * Booknetic_Mpdf::SCALE); // line to TL
		...
		foreach ($this->pageBackgrounds as $bl => $pbs) {
			...
			foreach ($pbs as $pb) {
				...
				if ((!isset($pb['image_id']) && !isset($pb['gradient'])) || isset($pb['shadowonly'])) { // Background colour or boxshadow
					...
					$s .= sprintf('%.3F %.3F %.3F %.3F re f Q', $pb['x'] * Booknetic_Mpdf::SCALE, ($this->h - $pb['y']) * Booknetic_Mpdf::SCALE, $pb['w'] * Booknetic_Mpdf::SCALE, -$pb['h'] * Booknetic_Mpdf::SCALE) . "\n";
			...
			foreach ($pbs as $pb) {
				...
				if (isset($pb['gradient']) && $pb['gradient']) {
					...
					$x = $pb['x'] * Booknetic_Mpdf::SCALE;
					$y = ($this->h - $pb['y']) * Booknetic_Mpdf::SCALE;
					$w = $pb['w'] * Booknetic_Mpdf::SCALE;
					$h = -$pb['h'] * Booknetic_Mpdf::SCALE;
					...
					if ($this->writingHTMLfooter || $this->writingHTMLheader) { // Write each (tiles) image rather than use as a pattern
						...
						$iw = $pb['orig_w'] / Booknetic_Mpdf::SCALE;
						$ih = $pb['orig_h'] / Booknetic_Mpdf::SCALE;
						...
						for ($xi = 0; $xi < $nx; $xi++) {
							for ($yi = 0; $yi < $ny; $yi++) {
								...
								$s .= sprintf("q %s %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q", $opac, $iw * Booknetic_Mpdf::SCALE, $ih * Booknetic_Mpdf::SCALE, $x * Booknetic_Mpdf::SCALE, ($this->h - ($y + $ih)) * Booknetic_Mpdf::SCALE, $pb['image_id']) . "\n";
							}
						}
		...
		foreach ($this->tableBackgrounds as $bl => $pbs) {
			foreach ($pbs as $pb) {
				if ((!isset($pb['gradient']) || !$pb['gradient']) && (!isset($pb['image_id']) || !$pb['image_id'])) {
					...
					$s .= sprintf('%.3F %.3F %.3F %.3F re %s Q', $pb['x'] * Booknetic_Mpdf::SCALE, ($this->h - $pb['y']) * Booknetic_Mpdf::SCALE, $pb['w'] * Booknetic_Mpdf::SCALE, -$pb['h'] * Booknetic_Mpdf::SCALE, 'f') . "\n";
				}
				if (isset($pb['gradient']) && $pb['gradient']) {
					if (isset($pb['clippath']) && $pb['clippath']) {
						$s .= $pb['clippath'] . "\n";
	...
	function AddPage(
		...
		if ($this->state > 0 && $this->page < count($this->pages)) {
			...
			$this->LineWidth = $lw;
			$this->writer->write(sprintf('%.3F w', $lw * Booknetic_Mpdf::SCALE));
	...
	function AddSpotColorsFromFile($file)
	{
		$colors = @file($file);
		if (!$colors) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException("Cannot load spot colors file - " . $file);
		}
	...
	// mPDF 5.7.1
		...
		$w += $kashida / Booknetic_Mpdf::SCALE;
	...
	function SetLineWidth($width)
	{
		...
		$lwout = (sprintf('%.3F w', $width * Booknetic_Mpdf::SCALE));
	...
	{
		// Draw a line
		$this->writer->write(sprintf('%.3F %.3F m %.3F %.3F l S', $x1 * Booknetic_Mpdf::SCALE, ($this->h - $y1) * Booknetic_Mpdf::SCALE, $x2 * Booknetic_Mpdf::SCALE, ($this->h - $y2) * Booknetic_Mpdf::SCALE));
	}
	...
	function Arrow($x1, $y1, $x2, $y2, $headsize = 3, $fill = 'B', $angle = 25)
	{
		...
		$s = '';
		$s .= sprintf('%.3F %.3F m %.3F %.3F l S', $x1 * Booknetic_Mpdf::SCALE, ($this->h - $y1) * Booknetic_Mpdf::SCALE, $x5 * Booknetic_Mpdf::SCALE, $y5 * Booknetic_Mpdf::SCALE);
		$this->writer->write($s);

		$s = '';
		$s .= sprintf('%.3F %.3F m %.3F %.3F l %.3F %.3F l %.3F %.3F l %.3F %.3F l ', $x5 * Booknetic_Mpdf::SCALE, $y5 * Booknetic_Mpdf::SCALE, $x3 * Booknetic_Mpdf::SCALE, $y3 * Booknetic_Mpdf::SCALE, $x2 * Booknetic_Mpdf::SCALE, ($this->h - $y2) * Booknetic_Mpdf::SCALE, $x4 * Booknetic_Mpdf::SCALE, $y4 * Booknetic_Mpdf::SCALE, $x5 * Booknetic_Mpdf::SCALE, $y5 * Booknetic_Mpdf::SCALE);
		$s .= $fill;
		$this->writer->write($s);
	}
	...
	function Rect($x, $y, $w, $h, $style = '')
	{
		...
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F re %s', $x * Booknetic_Mpdf::SCALE, ($this->h - $y) * Booknetic_Mpdf::SCALE, $w * Booknetic_Mpdf::SCALE, -$h * Booknetic_Mpdf::SCALE, $op));
	}
	...
	function AddFont($family, $style = '')
	{
		...
		if ($this->usingCoreFont) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException("mPDF Error - problem with Font management");
		}
		...
		if (!isset($this->fontdata[$family][$stylekey]) || !$this->fontdata[$family][$stylekey]) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('Font "%s%s%s" is not supported', $family, $style ? ' - ' : '', $style));
		}
	...
	function SetFont($family, $style = '', $size = 0, $write = true, $forcewrite = false)
	{
		...
		if ($family == '') {
			if ($this->FontFamily) {
				...
				throw new \Booknetic_Mpdf\Booknetic_MpdfException("No font or default font set!");
			}
		}
		...
		if (($family == 'csymbol') || ($family == 'czapfdingbats') || ($family == 'ctimes') || ($family == 'ccourier') || ($family == 'chelvetica')) {
			if ($this->PDFA || $this->PDFX) {
				if ($family == 'csymbol' || $family == 'czapfdingbats') {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException("Symbol and Zapfdingbats cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a).");
				}
		...
		if (!$this->onlyCoreFonts && !$this->usingCoreFont) {
			...
			$this->FontSize = $size / Booknetic_Mpdf::SCALE;
			...
			if ($this->PDFA || $this->PDFX) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException('Core Adobe fonts cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a) - cannot use option to use core fonts.');
			}
			...
			if (!isset($this->fonts[$fontkey])) {
				...
				if (isset($this->CoreFonts[$fontkey])) {
					...
					if (!isset($cw)) {
						throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('Could not include font metric file "%s"', $file));
					}
					$i = count($this->fonts) + $this->extraFontSubsets + 1;
					...
					throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('Font %s not defined', $fontkey));
				}
			}
	...
	{
		$l = [$x * Booknetic_Mpdf::SCALE, $this->hPt - $y * Booknetic_Mpdf::SCALE, $w * Booknetic_Mpdf::SCALE, $h * Booknetic_Mpdf::SCALE, $link];
	...
	function Text($x, $y, $txt, $OTLdata = [], $textvar = 0, $aixextra = '', $coordsys = '', $return = false)
	{
		...
		if ($this->falseBoldWeight && strpos($this->ReqFontStyle, "B") !== false && strpos($this->FontStyle, "B") === false) {
			$s .= '2 Tr 1 J 1 j ';
			$s .= sprintf('%.3F w ', ($this->FontSize / 130) * Booknetic_Mpdf::SCALE * $this->falseBoldWeight);
		...
		$py = $y;
		if ($coordsys != 'SVG') {
			$px = $x * Booknetic_Mpdf::SCALE;
			$py = ($this->h - $y) * Booknetic_Mpdf::SCALE;
		}
		...
		if (($textvar & TextVars::FD_UNDERLINE) && $txt != '') { // mPDF 5.7.1
			...
			$olw = $this->LineWidth;
			$s .= ' ' . (sprintf(' %.3F w', $ut * Booknetic_Mpdf::SCALE));
			$s .= ' ' . $this->_dounderline($x, $y + $adjusty, $txt, $OTLdata, $textvar);
			$s .= ' ' . (sprintf(' %.3F w', $olw * Booknetic_Mpdf::SCALE));
			if ($this->FillColor != $c) {
				$s.= ' ' . $this->FillColor . ' ';
			}
		}
	...
	function Cell($w, $h = 0, $txt = '', $border = 0, $ln = 0, $align = '', $fill = 0, $link = '', $currentx = 0, $lcpaddingL = 0, $lcpaddingR = 0, $valign = 'M', $spanfill = 0, $exactWidth = false, $OTLdata = false, $textvar = 0, $lineBox = false)
	{
		...
		if ($fill == 1 || $border == 1 || !empty($this->spanborddet)) {
			...
			if (!empty($this->spanborddet)) {
				...
				if ($fill == 1) {
					$s .= sprintf('%.3F %.3F %.3F %.3F re f ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bg_boxtop + $tbw) * Booknetic_Mpdf::SCALE, ($w + $lbw + $rbw) * Booknetic_Mpdf::SCALE, (-$bg_boxheight - $tbw - $bbw) * Booknetic_Mpdf::SCALE);
				}
				...
				if ($tbw) {
					...
					if ($this->spanborddet['T']['style'] == 'dashed') {
						$s .= sprintf(' 0 j 0 J [%.3F %.3F] 0 d ', $tbw * $dashon * Booknetic_Mpdf::SCALE, $tbw * $dashoff * Booknetic_Mpdf::SCALE);
					} elseif ($this->spanborddet['T']['style'] == 'dotted') {
						$s .= sprintf(' 1 j 1 J [%.3F %.3F] %.3F d ', 0.001, $tbw * $dot * Booknetic_Mpdf::SCALE, -$tbw / 2 * Booknetic_Mpdf::SCALE);
						$short = $tbw / 2;
					} else {
						$s .= ' 0 j 0 J [] 0 d ';
					}
					...
					if ($this->spanborddet['T']['style'] != 'dotted') {
						$s .= 'q ';
						$s .= sprintf('%.3F %.3F m ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w + $rbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop) * Booknetic_Mpdf::SCALE);
					...
					if ($this->spanborddet['T']['style'] == 'double') {
						$s .= sprintf(' %s %.3F w ', $c, $tbw / 3 * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw * 5 / 6) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw * 5 / 6) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw / 6) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw / 6) * Booknetic_Mpdf::SCALE);
					} elseif ($this->spanborddet['T']['style'] == 'dotted') {
						$s .= sprintf(' %s %.3F w ', $c, $tbw * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw / 2) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw - $short) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw / 2) * Booknetic_Mpdf::SCALE);
					} else {
						$s .= sprintf(' %s %.3F w ', $c, $tbw * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw / 2) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw - $short) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw / 2) * Booknetic_Mpdf::SCALE);
					}
				...
				if ($bbw) {
					...
					if ($this->spanborddet['B']['style'] == 'dashed') {
						$s .= sprintf(' 0 j 0 J [%.3F %.3F] 0 d ', $bbw * $dashon * Booknetic_Mpdf::SCALE, $bbw * $dashoff * Booknetic_Mpdf::SCALE);
					} elseif ($this->spanborddet['B']['style'] == 'dotted') {
						$s .= sprintf(' 1 j 1 J [%.3F %.3F] %.3F d ', 0.001, $bbw * $dot * Booknetic_Mpdf::SCALE, -$bbw / 2 * Booknetic_Mpdf::SCALE);
						$short = $bbw / 2;
					} else {
						$s .= ' 0 j 0 J [] 0 d ';
					}
					...
					if ($this->spanborddet['B']['style'] != 'dotted') {
						$s .= 'q ';
						$s .= sprintf('%.3F %.3F m ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w + $rbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom) * Booknetic_Mpdf::SCALE);
						$s .= ' h W n '; // Ends path no-op & Sets the clipping path
					}
					...
					if ($this->spanborddet['B']['style'] == 'double') {
						$s .= sprintf(' %s %.3F w ', $c, $bbw / 3 * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw / 6) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw - $short) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw / 6) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw * 5 / 6) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw - $short) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw * 5 / 6) * Booknetic_Mpdf::SCALE);
					} elseif ($this->spanborddet['B']['style'] == 'dotted') {
						$s .= sprintf(' %s %.3F w ', $c, $bbw * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw / 2) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw - $short) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw / 2) * Booknetic_Mpdf::SCALE);
					} else {
						$s .= sprintf(' %s %.3F w ', $c, $bbw * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw / 2) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw - $short) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw / 2) * Booknetic_Mpdf::SCALE);
					}
				...
				if ($lbw) {
					...
					if ($this->spanborddet['L']['style'] == 'dashed') {
						$s .= sprintf(' 0 j 0 J [%.3F %.3F] 0 d ', $lbw * $dashon * Booknetic_Mpdf::SCALE, $lbw * $dashoff * Booknetic_Mpdf::SCALE);
					} elseif ($this->spanborddet['L']['style'] == 'dotted') {
						$s .= sprintf(' 1 j 1 J [%.3F %.3F] %.3F d ', 0.001, $lbw * $dot * Booknetic_Mpdf::SCALE, -$lbw / 2 * Booknetic_Mpdf::SCALE);
						$short = $lbw / 2;
					} else {
						$s .= ' 0 j 0 J [] 0 d ';
					}
					...
					if ($this->spanborddet['L']['style'] != 'dotted') {
						...
						$s .= sprintf('%.3F %.3F l ', ($this->x) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x - $lbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE);
						$s .= ' h W n '; // Ends path no-op & Sets the clipping path
					}

					$c = $this->SetDColor($this->spanborddet['L']['c'], true);
					if ($this->spanborddet['L']['style'] == 'double') {
						$s .= sprintf(' %s %.3F w ', $c, $lbw / 3 * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw / 6) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE, ($this->x - $lbw / 6) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw + $short) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw * 5 / 6) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE, ($this->x - $lbw * 5 / 6) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw + $short) * Booknetic_Mpdf::SCALE);
					} elseif ($this->spanborddet['L']['style'] == 'dotted') {
						$s .= sprintf(' %s %.3F w ', $c, $lbw * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw / 2) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE, ($this->x - $lbw / 2) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw + $short) * Booknetic_Mpdf::SCALE);
					} else {
						$s .= sprintf(' %s %.3F w ', $c, $lbw * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x - $lbw / 2) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE, ($this->x - $lbw / 2) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw + $short) * Booknetic_Mpdf::SCALE);
					}
				...
				if ($rbw) {
					...
					if ($this->spanborddet['R']['style'] == 'dashed') {
						$s .= sprintf(' 0 j 0 J [%.3F %.3F] 0 d ', $rbw * $dashon * Booknetic_Mpdf::SCALE, $rbw * $dashoff * Booknetic_Mpdf::SCALE);
					} elseif ($this->spanborddet['R']['style'] == 'dotted') {
						$s .= sprintf(' 1 j 1 J [%.3F %.3F] %.3F d ', 0.001, $rbw * $dot * Booknetic_Mpdf::SCALE, -$rbw / 2 * Booknetic_Mpdf::SCALE);
						$short = $rbw / 2;
					} else {
						$s .= ' 0 j 0 J [] 0 d ';
					}
					...
					if ($this->spanborddet['R']['style'] != 'dotted') {
						$s .= 'q ';
						$s .= sprintf('%.3F %.3F m ', ($this->x + $w + $rbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F l ', ($this->x + $w + $rbw) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE);
						$s .= ' h W n '; // Ends path no-op & Sets the clipping path
					...
					if ($this->spanborddet['R']['style'] == 'double') {
						$s .= sprintf(' %s %.3F w ', $c, $rbw / 3 * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x + $w + $rbw / 6) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw / 6) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw + $short) * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x + $w + $rbw * 5 / 6) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw * 5 / 6) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw + $short) * Booknetic_Mpdf::SCALE);
					} elseif ($this->spanborddet['R']['style'] == 'dotted') {
						$s .= sprintf(' %s %.3F w ', $c, $rbw * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x + $w + $rbw / 2) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw / 2) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw + $short) * Booknetic_Mpdf::SCALE);
					} else {
						$s .= sprintf(' %s %.3F w ', $c, $rbw * Booknetic_Mpdf::SCALE);
						$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($this->x + $w + $rbw / 2) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop + $tbw) * Booknetic_Mpdf::SCALE, ($this->x + $w + $rbw / 2) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxbottom - $bbw + $short) * Booknetic_Mpdf::SCALE);
					}
				...
				$s .= sprintf('%.3F %.3F %.3F %.3F re %s ', $this->x * Booknetic_Mpdf::SCALE, ($this->h - $bg_boxtop) * Booknetic_Mpdf::SCALE, $w * Booknetic_Mpdf::SCALE, -$bg_boxheight * Booknetic_Mpdf::SCALE, $op);
			}
		}
		...
		if (is_string($border)) { // If "border", does not come from WriteFlowingBlock or FinishFlowingBlock
			...
			if (is_int(strpos($border, 'L'))) {
				$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', $x * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop) * Booknetic_Mpdf::SCALE, $x * Booknetic_Mpdf::SCALE, ($this->h - ($bord_boxbottom)) * Booknetic_Mpdf::SCALE);
			}

			if (is_int(strpos($border, 'T'))) {
				$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', $x * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop) * Booknetic_Mpdf::SCALE, ($x + $w) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop) * Booknetic_Mpdf::SCALE);
			}

			if (is_int(strpos($border, 'R'))) {
				$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', ($x + $w) * Booknetic_Mpdf::SCALE, ($this->h - $bord_boxtop) * Booknetic_Mpdf::SCALE, ($x + $w) * Booknetic_Mpdf::SCALE, ($this->h - ($bord_boxbottom)) * Booknetic_Mpdf::SCALE);
			}

			if (is_int(strpos($border, 'B'))) {
				$s .= sprintf('%.3F %.3F m %.3F %.3F l S ', $x * Booknetic_Mpdf::SCALE, ($this->h - ($bord_boxbottom)) * Booknetic_Mpdf::SCALE, ($x + $w) * Booknetic_Mpdf::SCALE, ($this->h - ($bord_boxbottom)) * Booknetic_Mpdf::SCALE);
			}
		}
		...
		if ($txt != '') {
			...
			if ($exactWidth) {
				...
				$stringWidth = $this->GetStringWidth($txt, true, $OTLdata, $textvar) + ( $this->charspacing * mb_strlen($txt, $this->mb_enc) / Booknetic_Mpdf::SCALE ) + ( $this->ws * mb_substr_count($txt, ' ', $this->mb_enc) / Booknetic_Mpdf::SCALE );
			}
			...
			if (isset($this->textparam['outline-s']) && $this->textparam['outline-s'] && !($textvar & TextVars::FC_SMALLCAPS)) { // mPDF 5.7.1
				$s .=' ' . sprintf('%.3F w', $this->LineWidth * Booknetic_Mpdf::SCALE) . ' ';
				$s .=" $this->DrawColor ";
				$s .=" 2 Tr ";
			...
			$px = ($this->x + $dx) * Booknetic_Mpdf::SCALE;
			$py = ($this->h - ($this->y + $glyphYorigin - $baseline_shift)) * Booknetic_Mpdf::SCALE;
			...
			if ($textvar & TextVars::FD_UNDERLINE) { // mPDF 5.7.1	// mPDF 6
				...
				$sub .= ' ' . (sprintf(' %.3F w 0 j 0 J ', $ut * Booknetic_Mpdf::SCALE));
				$sub .= ' ' . $this->_dounderline($this->x + $dx, $this->y + $ubaseline + $adjusty, $txt, $OTLdata, $textvar);
				$sub .= ' ' . (sprintf(' %.3F w 2 j 2 J ', $olw * Booknetic_Mpdf::SCALE));
			...
			if ($this->textshadow) {  // First to process is last in CSS comma separated shadows
				foreach ($this->textshadow as $ts) {
					...
					$s .= sprintf(' 1 0 0 1 %.4F %.4F cm', $ts['x'] * Booknetic_Mpdf::SCALE, -$ts['y'] * Booknetic_Mpdf::SCALE) . "\n";
					$s .= $sub;
					$s .= ' Q ';
				}
			}
	...
	function _kern($txt, $mode, $aix, $x, $y)
	{
		if ($mode == 'MBTw') { // Multibyte requiring word spacing
			...
			$space = $this->writer->utf8ToUtf16BigEndian($space, false);
			$space = $this->writer->escape($space);
			$s = sprintf(' BT ' . $aix, $x * Booknetic_Mpdf::SCALE, ($this->h - $y) * Booknetic_Mpdf::SCALE);
			$t = explode(' ', $txt);
			for ($i = 0; $i < count($t); $i++) {
				$tx = $t[$i];
			...
			$tj .= ')';
			$s .= sprintf(' BT ' . $aix . ' [%s] TJ ET ', $x * Booknetic_Mpdf::SCALE, ($this->h - $y) * Booknetic_Mpdf::SCALE, $tj);
		} else { // CORE Font
			$s = '';
			$tj = '(';
			$l = strlen($txt);
				...
				if ($l > $wmax) {
					...
					if ($sep == -1) { // Only one word
						...
						if ($align == 'J') {
							...
							list($charspacing, $ws, $kashida) = $this->GetJspacing($nb_carac, $nb_spaces, ((($wmax) - $len_ligne) * Booknetic_Mpdf::SCALE), $inclCursive, $tmpOTLdata);
							$this->SetSpacing($charspacing, $ws);
							//////////////////////////////////////////
						}
						if (isset($OTLdata)) {
				...
				if ($l > $wmax) {
					...
					if ($sep == -1) {
						...
						if ($align == 'J') {
							...
							$tmpOTLdata = [];
							list($charspacing, $ws, $kashida) = $this->GetJspacing($nb_carac, $nb_spaces, ((($wmax) - $len_ligne) * Booknetic_Mpdf::SCALE), false, $tmpOTLdata);
							$this->SetSpacing($charspacing, $ws);
							//////////////////////////////////////////
						}
	...
	function newFlowingBlock($w, $h, $a = '', $is_table = false, $blockstate = 0, $newblock = true, $blockdir = 'ltr', $table_draft = false)
	{
		...
		$this->flowingBlockAttr['width'] = ($w * Booknetic_Mpdf::SCALE);
	...
	function finishFlowingBlock($endofblock = false, $next = '')
	{
		...
		if ($is_table) {
			...
			$paddingL = ($ipaddingL * Booknetic_Mpdf::SCALE);
			$paddingR = ($ipaddingR * Booknetic_Mpdf::SCALE);
		...
		foreach ($content as $k => $chunk) {
			...
			if (!isset($this->objectbuffer[$k]) || (isset($this->objectbuffer[$k]) && !$this->objectbuffer[$k])) {
				...
				$contentWidth += $this->GetStringWidth($chunk, true, (isset($cOTLdata[$k]) ? $cOTLdata[$k] : false), $this->textvar) * Booknetic_Mpdf::SCALE;
				...
				if ($this->objectbuffer[$k]['type'] == 'image' && isset($this->objectbuffer[$k]['listmarker']) && $this->objectbuffer[$k]['listmarker'] && $this->objectbuffer[$k]['listmarkerposition'] == 'outside') {
					...
					$contentWidth += $this->objectbuffer[$k]['OUTER-WIDTH'] * Booknetic_Mpdf::SCALE;
				}
			}
		}
		...
		if ($blockdir == 'ltr' && strpos($lastfontreqstyle, "I") !== false && strpos($lastfontstyle, "I") === false) { // Artificial italic
			$lastitalic = $this->FontSize * 0.15 * Booknetic_Mpdf::SCALE;
		} else {
			$lastitalic = 0;
		}
		...
		if (($newblock) && ($blockstate == 1 || $blockstate == 3) && isset($this->blk[$this->blklvl]['text_indent']) && ($lineCount == 0) && (!$is_table) && ($align != 'C')) {
			...
			$WidthCorrection = ($ti * Booknetic_Mpdf::SCALE);
		}
		...
		if ($content) {
				...
				if (count($cOTLdata)) {
					...
					if ($aord == count($chunkorder) - 1 && isset($cOTLdata[$aord]['group'])) { // Last chunk on line
						...
						if (isset($cOTLdata[$aord]['GPOSinfo'][$nGPOS]['XAdvanceL']) || isset($cOTLdata[$aord]['GPOSinfo'][$nGPOS]['XAdvanceR'])) {
							...
							$w *= ($this->FontSize / 1000);
							$contentWidth -= $w * Booknetic_Mpdf::SCALE;
			...
			if ((((($contentWidth + $lastitalic) > $maxWidth) && ($content[(count($chunkorder) - 1)] != ' ') ) ||
				...
				// WORD SPACING
				list($jcharspacing, $jws, $jkashida) = $this->GetJspacing($nb_carac, $nb_spaces, ($maxWidth - $lastitalic - $contentWidth - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * Booknetic_Mpdf::SCALE) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * Booknetic_Mpdf::SCALE) )), $inclCursive, $cOTLdata);
				...
				if (preg_match("/[" . $this->CJKoverflow . "]/u", $hanger)) {
					...
					$contentWidth -= $this->GetStringWidth($hanger) * Booknetic_Mpdf::SCALE;
					$nb_carac -= 1;
					list($jcharspacing, $jws, $jkashida) = $this->GetJspacing($nb_carac, $nb_spaces, ($maxWidth - $lastitalic - $contentWidth - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * Booknetic_Mpdf::SCALE) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * Booknetic_Mpdf::SCALE) )), $inclCursive, $cOTLdata);
				}
			} /* -- END CJK-FONTS -- */
			...
			elseif ($contentWidth < ($maxWidth - $lastitalic - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * Booknetic_Mpdf::SCALE) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * Booknetic_Mpdf::SCALE))) && !$this->fixedlSpacing) {
				...
				$check = $maxWidth - $lastitalic - $WidthCorrection - $contentWidth - (($this->cMarginL + $this->cMarginR) * Booknetic_Mpdf::SCALE) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * Booknetic_Mpdf::SCALE) ) - ( $jcharspacing * $nb_carac) - ( $jws * $nb_spaces);
				if ($check <= 0) {
					$jcharspacing = 0;
					$jws = 0;
				}
			...
			$empty = $maxWidth - $lastitalic - $WidthCorrection - $contentWidth - (($this->cMarginL + $this->cMarginR) * Booknetic_Mpdf::SCALE) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * Booknetic_Mpdf::SCALE) );
			...
			foreach ($chunkorder as $aord => $k) { // mPDF 5.7
				...
				$this->SetSpacing(($this->fixedlSpacing * Booknetic_Mpdf::SCALE) + $jcharspacing, ($this->fixedlSpacing + $this->minwSpacing) * Booknetic_Mpdf::SCALE + $jws);
				$this->fixedlSpacing = false;
				$this->minwSpacing = 0;
				...
				$stringWidth += ( $this->charspacing * $nch / Booknetic_Mpdf::SCALE );

				$stringWidth += ( $this->ws * mb_substr_count($chunk, ' ', $this->mb_enc) / Booknetic_Mpdf::SCALE );
	...
	function printobjectbuffer($is_table = false, $blockdir = false)
	{
		...
		foreach ($this->objectbuffer as $ib => $objattr) {
			...
			if ($objattr['type'] == 'image') {
				...
				$sx = $objattr['orig_w'] ? ($objattr['INNER-WIDTH'] * Booknetic_Mpdf::SCALE / $objattr['orig_w']) : INF;
				$sy = $objattr['orig_h'] ? ($objattr['INNER-HEIGHT'] * Booknetic_Mpdf::SCALE / $objattr['orig_h']) : INF;
				...
				if ($rotate == 90) {
					...
					$sx = $obiw * Booknetic_Mpdf::SCALE / $objattr['orig_h'];
					$sy = $obih * Booknetic_Mpdf::SCALE / $objattr['orig_w'];
				...
				if (isset($objattr['itype']) && $objattr['itype'] == 'wmf') {
					$outstring = sprintf('q ' . $tr . $tr2 . '%.3F 0 0 %.3F %.3F %.3F cm /FO%d Do Q', $sx, -$sy, $objattr['INNER-X'] * Booknetic_Mpdf::SCALE - $sx * $objattr['wmf_x'], (($this->h - $objattr['INNER-Y']) * Booknetic_Mpdf::SCALE) + $sy * $objattr['wmf_y'], $objattr['ID']); // mPDF 5.7.3 TRANSFORMS
				} else { 				/* -- END IMAGES-WMF -- */
					if (isset($objattr['itype']) && $objattr['itype'] == 'svg') {
						$outstring = sprintf('q ' . $tr . $tr2 . '%.3F 0 0 %.3F %.3F %.3F cm /FO%d Do Q', $sx, -$sy, $objattr['INNER-X'] * Booknetic_Mpdf::SCALE - $sx * $objattr['wmf_x'], (($this->h - $objattr['INNER-Y']) * Booknetic_Mpdf::SCALE) + $sy * $objattr['wmf_y'], $objattr['ID']); // mPDF 5.7.3 TRANSFORMS
					} else {
						$outstring = sprintf("q " . $tr . $tr2 . "%.3F 0 0 %.3F %.3F %.3F cm " . $gradmask . "/I%d Do Q", $obiw * Booknetic_Mpdf::SCALE, $obih * Booknetic_Mpdf::SCALE, $objattr['INNER-X'] * Booknetic_Mpdf::SCALE, ($this->h - ($objattr['INNER-Y'] + $obih )) * Booknetic_Mpdf::SCALE, $objattr['ID']); // mPDF 5.7.3 TRANSFORMS
					}
				}
			...
			if ($objattr['type'] === 'barcode') {
				...
				if (in_array($objattr['btype'], $barcodeTypes, true)) {
					...
					if (!class_exists('Booknetic_Mpdf\QrCode\QrCode')) {
						throw new \Booknetic_Mpdf\Booknetic_MpdfException('Class Booknetic_Mpdf\QrCode\QrCode does not exists. Install the package from Packagist with "composer require mpdf/qrcode"');
					}
					...
					$out = new QrCode\Output\Booknetic_Mpdf();
					$out->output(
						$qrcode,
						$this,
						$objattr['INNER-X'],
	...
	function _printListBullet($x, $y, $size, $type, $color)
	{
		...
		if ($type == 'square') {
			...
			$this->writer->write(sprintf('%.3F %.3F %.3F %.3F re f', ($x - $size / 2) * Booknetic_Mpdf::SCALE, ($this->h - $y + $size / 2) * Booknetic_Mpdf::SCALE, ($size) * Booknetic_Mpdf::SCALE, (-$size) * Booknetic_Mpdf::SCALE));
		} elseif ($type == 'disc') {
	...
	// mPDF 5.7.1
		...
		if (substr($s, 0, 3) == "\xbb\xa4\xac") { // identifier has been identified!
			...
			$h_corr = 0;
			if ($is_table) { // *TABLES*
				$maximumW = ($maxWidth / Booknetic_Mpdf::SCALE) - ($this->cellPaddingL + $this->cMarginL + $this->cellPaddingR + $this->cMarginR);  // *TABLES*
			} // *TABLES*
			else { // *TABLES*
				if (($newblock) && ($blockstate == 1 || $blockstate == 3) && ($lineCount == 0) && (!$is_table)) {
					$h_corr = $this->blk[$this->blklvl]['padding_top'] + $this->blk[$this->blklvl]['border_top']['w'];
				}
				$maximumW = ($maxWidth / Booknetic_Mpdf::SCALE) - ($this->blk[$this->blklvl]['padding_left'] + $this->blk[$this->blklvl]['border_left']['w'] + $this->blk[$this->blklvl]['padding_right'] + $this->blk[$this->blklvl]['border_right']['w'] + $fpaddingL + $fpaddingR );
			} // *TABLES*
			$objattr = $this->inlineObject($objattr['type'], $this->lMargin + $fpaddingL + ($contentWidth / Booknetic_Mpdf::SCALE), ($this->y + $h_corr), $objattr, $this->lMargin, ($contentWidth / Booknetic_Mpdf::SCALE), $maximumW, $stackHeight, true, $is_table);
			...
			if ($objattr['type'] == 'image' && isset($objattr['listmarker']) && $objattr['listmarker'] && $objattr['listmarkerposition'] == 'outside') {
				...
				$contentWidth += ($objattr['OUTER-WIDTH'] * Booknetic_Mpdf::SCALE);
			}
			return;
		}
		...
		for ($i = 0; $i < $clen; $i++) {
			...
			if ($this->usingCoreFont) {
				$c = $s[$i];
				// Soft Hyphens chr(173)
				$cw = ($this->GetCharWidthCore($c) * Booknetic_Mpdf::SCALE);
				...
				$c = mb_substr($s, $i, 1, $this->mb_enc);
				$cw = ($this->GetCharWidthNonCore($c, false) * Booknetic_Mpdf::SCALE);
				// mPDF 5.7.1
				// Use OTL GPOS
				if (isset($this->CurrentFont['useOTL']) && ($this->CurrentFont['useOTL'] & 0xFF)) {
					...
					if (isset($sOTLdata['GPOSinfo'][$i]['XAdvanceL'])) {
						$cw += $sOTLdata['GPOSinfo'][$i]['XAdvanceL'] * (1000 / $this->CurrentFont['unitsPerEm']) * ($this->FontSize / 1000) * Booknetic_Mpdf::SCALE;
					}
				}
				if (($this->textvar & TextVars::FC_KERNING) && $i > 0) { // mPDF 5.7.1
			...
			if ($i == 0) {
				$cw += $lbw * Booknetic_Mpdf::SCALE;
				$contentB[(count($contentB) - 1)] .= 'L';
			}
			if ($i == ($clen - 1)) {
				$cw += $rbw * Booknetic_Mpdf::SCALE;
			...
			foreach ($this->objectbuffer as $k => $objattr) {   // mPDF 6 DOTTAB
				if ($objattr['type'] == 'dottab') {
					$WidthCorrection -= ($objattr['outdent'] * Booknetic_Mpdf::SCALE);
					break;
				}
			}
			...
			if (( $contentWidth + $cw > $maxWidth - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * Booknetic_Mpdf::SCALE) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * Booknetic_Mpdf::SCALE) ) + 0.001)) {// 0.001 is to correct for deviations converting mm=>pts
				...
				foreach ($content as $k => $chunk) {
					if (isset($this->objectbuffer[$k]) && $this->objectbuffer[$k]) {
						...
						$contentWidth += $this->GetStringWidth($chunk, true, (isset($cOTLdata[$k]) ? $cOTLdata[$k] : false), $this->textvar) * Booknetic_Mpdf::SCALE;  // mPDF 5.7.1
						if (!empty($this->spanborddet)) {
							if (isset($this->spanborddet['L']['w']) && strpos($contentB[$k], 'L') !== false) {
								$contentWidth += $this->spanborddet['L']['w'] * Booknetic_Mpdf::SCALE;
							}
							if (isset($this->spanborddet['R']['w']) && strpos($contentB[$k], 'R') !== false) {
								$contentWidth += $this->spanborddet['R']['w'] * Booknetic_Mpdf::SCALE;
							}
						}
					}
				}
				...
				if (!$table_draft) {
					...
					if (($align == 'J' && !$CJKoverflow) || (($contentWidth + $lastitalic > $maxWidth - $WidthCorrection - (($this->cMarginL + $this->cMarginR) * Booknetic_Mpdf::SCALE) - ($paddingL + $paddingR + (($fpaddingL + $fpaddingR) * Booknetic_Mpdf::SCALE) ) + 0.001) && (!$CJKoverflow || ($CJKoverflow && !$this->allowCJKoverflow))) || $CJKoverflow && $align == 'J' && $this->allowCJKoverflow && $hanger && $this->CJKforceend) {   // 0.001 is to correct for deviations converting mm=>pts
					...
					foreach ($chunkorder as $aord => $k) { // mPDF 5.7
						...
						if ($aord == $arraysize - 1) {
							$stringWidth -= ( $this->charspacing / Booknetic_Mpdf::SCALE );
							if ($this->checkCJK && $CJKoverflow && $align == 'J' && $this->allowCJKoverflow && $hanger && $this->CJKforceend) {
								// force-end overhang
				...
				if (!empty($savedObj)) {
					...
					$contentB[] = '';
					$cOTLdata[] = []; // mPDF 5.7.1
					$contentWidth += $savedObj['OUTER-WIDTH'] * Booknetic_Mpdf::SCALE;
				}
				if (count($savedPreContent) > 0) {
					for ($ix = count($savedPreContent) - 1; $ix >= 0; $ix--) {
						...
						if ($ix > 0) {
							$contentWidth += $this->GetStringWidth($savedPreContent[$ix], true, (isset($savedPreOTLdata[$ix]) ? $savedPreOTLdata[$ix] : false), $this->textvar) * Booknetic_Mpdf::SCALE; // mPDF 5.7.1
				...
				$contentWidth += $this->GetStringWidth($currContent, false, (isset($cOTLdata[(count($cOTLdata) - 1)]) ? $cOTLdata[(count($cOTLdata) - 1)] : false), $this->textvar) * Booknetic_Mpdf::SCALE; // mPDF 5.7.1
	...
	function _SetTextRendering($mode)
	{
		if (!(($mode == 0) || ($mode == 1) || ($mode == 2))) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException("Text rendering mode should be 0, 1 or 2 (value : $mode)");
		}
		$tr = ($mode . ' Tr');
	...
	function Image($file, $x, $y, $w = 0, $h = 0, $type = '', $link = '', $paint = true, $constrain = true, $watermark = false, $shownoimg = true, $allowvector = true)
	{
		...
		if ($w == 0 and $h == 0) {
			...
			if ($info['type'] == 'wmf') {
				...
				$w = abs($info['w']) / (20 * Booknetic_Mpdf::SCALE);
				$h = abs($info['h']) / (20 * Booknetic_Mpdf::SCALE);
			} else { 			/* -- END IMAGES-WMF -- */
				if ($info['type'] == 'svg') {
					// returned SVG units are pts
					// divide by k to get user units (mm)
					$w = abs($info['w']) / Booknetic_Mpdf::SCALE;
					$h = abs($info['h']) / Booknetic_Mpdf::SCALE;
				} else {
					// Put image at default image dpi
					$w = ($info['w'] / Booknetic_Mpdf::SCALE) * (72 / $this->img_dpi);
					$h = ($info['h'] / Booknetic_Mpdf::SCALE) * (72 / $this->img_dpi);
				}
			}
		}
		if ($w == 0) {
		...
		if ($watermark) {
			...
			/* -- IMAGES-WMF -- */
			if ($info['type'] == 'wmf') {
				$sx = $w * Booknetic_Mpdf::SCALE / $info['w'];
				$sy = -$h * Booknetic_Mpdf::SCALE / $info['h'];
				$outstring = sprintf('q %.3F 0 0 %.3F %.3F %.3F cm /FO%d Do Q', $sx, $sy, $x * Booknetic_Mpdf::SCALE - $sx * $info['x'], (($this->h - $y) * Booknetic_Mpdf::SCALE) - $sy * $info['y'], $info['i']);
			} else { 			/* -- END IMAGES-WMF -- */
				if ($info['type'] == 'svg') {
					$sx = $w * Booknetic_Mpdf::SCALE / $info['w'];
					$sy = -$h * Booknetic_Mpdf::SCALE / $info['h'];
					...
					$outstring = sprintf("q %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q", $w * Booknetic_Mpdf::SCALE, $h * Booknetic_Mpdf::SCALE, $x * Booknetic_Mpdf::SCALE, ($this->h - ($y + $h)) * Booknetic_Mpdf::SCALE, $info['i']);
				}
			}
	...
	{
		if ($black and $white) {
			$s = sprintf('[%.3F %.3F] 0 d', $black * Booknetic_Mpdf::SCALE, $white * Booknetic_Mpdf::SCALE);
		} else {
			$s = '[] 0 d';
		}
	...
	function Output($name = '', $dest = '')
	{
		...
		if ($this->debug && error_get_last()) {
			...
			if (($e['type'] < 2048 && $e['type'] != 8) || (intval($e['type']) & intval(ini_get("error_reporting")))) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException(
		...
		if (($this->PDFA || $this->PDFX) && $this->encrypted) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('PDF/A1-b or PDF/X1-a does not permit encryption of documents.');
		}
		...
		if (count($this->PDFAXwarnings) && (($this->PDFA && !$this->PDFAauto) || ($this->PDFX && !$this->PDFXauto))) {
			...
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('PDFA/PDFX warnings generated. See log for further details');
		}
		...
		switch ($dest) {
			...
			case Destination::INLINE:
				...
				if ($this->debug && !$this->allow_output_buffering && ob_get_contents()) {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException('Output has already been sent from the script - PDF file generation aborted.');
				}
			...
			case Destination::DOWNLOAD:
				...
				if (headers_sent()) {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException('Data has already been sent to output, unable to output PDF file');
				}
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('Unable to create output file %s', $name));
				}
			...
			default:
				throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('Incorrect output destination %s', $dest));
		}
	...
	function _dochecks()
	{
		...
		if (1.1 == 1) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Do not alter the locale before including mPDF');
		}
		...
		if (ini_get('mbstring.func_overload')) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Booknetic_Mpdf cannot function properly with mbstring.func_overload enabled');
		}

		if (!function_exists('mb_substr')) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('mbstring extension must be loaded in order to run mPDF');
		}
	}
	...
	function _puthtmlheaders()
	{
		...
		for ($n = 1; $n <= $nb; $n++) {
			...
			if (isset($this->saveHTMLHeader[$n][$OE])) {
				...
				$os = '';
				if ($rotate) {
					$os .= sprintf('q 0 -1 1 0 0 %.3F cm ', ($this->w * Booknetic_Mpdf::SCALE));
					// To rotate the other way i.e. Header to left of page:
					// $os .= sprintf('q 0 1 -1 0 %.3F %.3F cm ',($this->h*Booknetic_Mpdf::SCALE), (($this->rMargin - $this->lMargin )*Booknetic_Mpdf::SCALE));
				}
				...
				foreach ($lks as $lk) {
					if ($rotate) {
						...
						$ax = $lk[0] / Booknetic_Mpdf::SCALE;
						$ay = $lk[1] / Booknetic_Mpdf::SCALE;
						$bx = $ay - ($lh / Booknetic_Mpdf::SCALE);
						$by = $this->w - $ax;
						$lk[0] = $bx * Booknetic_Mpdf::SCALE;
						$lk[1] = ($this->h - $by) * Booknetic_Mpdf::SCALE - $lw;
					}
					$this->PageLinks[$n][] = $lk;
				}
				/* -- FORMS -- */
	...
	function _dounderline($x, $y, $txt, $OTLdata = false, $textvar = 0)
	{
		...
		// WORD SPACING
		$w = ($this->GetStringWidth($txt, false, $OTLdata, $textvar) * Booknetic_Mpdf::SCALE) + ($this->charspacing * mb_strlen($txt, $this->mb_enc)) + ( $this->ws * mb_substr_count($txt, ' ', $this->mb_enc));
		// Draw a line
		return sprintf('%.3F %.3F m %.3F %.3F l S', $x * Booknetic_Mpdf::SCALE, ($this->h - $y) * Booknetic_Mpdf::SCALE, ($x * Booknetic_Mpdf::SCALE) + $w, ($this->h - $y) * Booknetic_Mpdf::SCALE);
	}
	...
	{
		if ($this->PDFA || $this->PDFX) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('PDFA and PDFX do not permit transparency, so mPDF does not allow Watermarks!');
		}
		...
		while ($loop == 0) {
			...
			$offset = ((sin(deg2rad($angle))) * ($szfont / Booknetic_Mpdf::SCALE));
	...
	function Rotate($angle, $x = -1, $y = -1)
	{
		...
		if ($angle != 0) {
			...
			$c = cos($angle);
			$s = sin($angle);
			$cx = $x * Booknetic_Mpdf::SCALE;
			$cy = ($this->h - $y) * Booknetic_Mpdf::SCALE;
	...
	function RoundedRect($x, $y, $w, $h, $r, $style = '')
	{
		...
		$MyArc = 4 / 3 * (sqrt(2) - 1);
		$this->writer->write(sprintf('%.3F %.3F m', ($x + $r) * Booknetic_Mpdf::SCALE, ($hp - $y) * Booknetic_Mpdf::SCALE));
		$xc = $x + $w - $r;
		$yc = $y + $r;
		$this->writer->write(sprintf('%.3F %.3F l', $xc * Booknetic_Mpdf::SCALE, ($hp - $y) * Booknetic_Mpdf::SCALE));

		$this->_Arc($xc + $r * $MyArc, $yc - $r, $xc + $r, $yc - $r * $MyArc, $xc + $r, $yc);
		$xc = $x + $w - $r;
		$yc = $y + $h - $r;
		$this->writer->write(sprintf('%.3F %.3F l', ($x + $w) * Booknetic_Mpdf::SCALE, ($hp - $yc) * Booknetic_Mpdf::SCALE));

		$this->_Arc($xc + $r, $yc + $r * $MyArc, $xc + $r * $MyArc, $yc + $r, $xc, $yc + $r);
		$xc = $x + $r;
		$yc = $y + $h - $r;
		$this->writer->write(sprintf('%.3F %.3F l', $xc * Booknetic_Mpdf::SCALE, ($hp - ($y + $h)) * Booknetic_Mpdf::SCALE));

		$this->_Arc($xc - $r * $MyArc, $yc + $r, $xc - $r, $yc + $r * $MyArc, $xc - $r, $yc);
		$xc = $x + $r;
		$yc = $y + $r;
		$this->writer->write(sprintf('%.3F %.3F l', ($x) * Booknetic_Mpdf::SCALE, ($hp - $yc) * Booknetic_Mpdf::SCALE));
	...
	{
		$h = $this->h;
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', $x1 * Booknetic_Mpdf::SCALE, ($h - $y1) * Booknetic_Mpdf::SCALE, $x2 * Booknetic_Mpdf::SCALE, ($h - $y2) * Booknetic_Mpdf::SCALE, $x3 * Booknetic_Mpdf::SCALE, ($h - $y3) * Booknetic_Mpdf::SCALE));
	}
	...
	function AddCIDFont($family, $style, $name, &$cw, $CMap, $registry, $desc)
	{
		...
		if (isset($this->fonts[$fontkey])) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException("Font already added: $family $style");
		}
		$i = count($this->fonts) + $this->extraFontSubsets + 1;
	...
	function AddCJKFont($family)
	{
		...
		if ($this->PDFA || $this->PDFX) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException("Adobe CJK fonts cannot be embedded in mPDF (required for PDFA1-b and PDFX/1-a).");
		}
		if ($family == 'big5') {
			$this->AddBig5Font();
		} elseif ($family == 'gb') {
	...
	function SetLineHeight($FontPt = '', $lh = '')
	{
		...
		$fs = $FontPt / Booknetic_Mpdf::SCALE;
		...
		foreach ($content as $k => $chunk) {
			if (isset($this->objectbuffer[$k])) {
				...
				if ($this->objectbuffer[$k]['type'] == 'listmarker') {
					...
					$oh = $font[$k]['size']; // == $this->objectbuffer[$k]['fontsize']/Booknetic_Mpdf::SCALE;
	...
	function TableHeaderFooter($content = '', $tablestartpage = '', $tablestartcolumn = '', $horf = 'H', $level = 0, $firstSpread = true, $finalSpread = true)
	{
		if (($horf == 'H' || $horf == 'F') && !empty($content)) { // mPDF 5.7.2
			...
			if (empty($content[$firstrow])) {
				if ($this->debug) {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException("<tfoot> must precede <tbody> in a table");
				} else {
					return;
				}
			}
				...
				foreach ($content[$i] as $tablehf) {
					...
					if (isset($tablehf['background-image']) && $paintcell) {
						if ($tablehf['background-image']['gradient'] && preg_match('/(-moz-)*(repeating-)*(linear|radial)-gradient/', $tablehf['background-image']['gradient'])) {
							...
							if ($this->ColActive) {
								...
								$this->writer->write(sprintf('q /Pattern cs /P%d scn %s %.3F %.3F %.3F %.3F re f Q', $n, $opac, $px * Booknetic_Mpdf::SCALE, ($this->h - $py) * Booknetic_Mpdf::SCALE, $pw * Booknetic_Mpdf::SCALE, -$ph * Booknetic_Mpdf::SCALE));
							} else {
					...
					if (!empty($textbuffer)) {
						...
						if ($R) {
							...
							$cellFontHeight = ($cellPtSize / Booknetic_Mpdf::SCALE);
							$opx = $this->x;
							$opy = $this->y;
							$angle = intval($R);
	...
	function WriteHTML($html, $mode = HTMLParserMode::DEFAULT_MODE, $init = true, $close = true)
	{
		...
		if (is_scalar($html) === false) {
			if (!is_object($html) || ! method_exists($html, '__toString')) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException('WriteHTML() requires $html be an integer, float, string, boolean or an object with the __toString() magic method.');
			}
		}

		// Check the mode is valid
		if (in_array($mode, HTMLParserMode::getAllModes(), true) === false) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('WriteHTML() requires $mode to be one of the modes defined in HTMLParserMode');
		}
	...
	function WriteFixedPosHTML($html, $x, $y, $w, $h, $overflow = 'visible', $bounding = [])
	{
		...
		if ($overflow == 'hidden') {
			...
			$this->writer->write("q");
			$ch = $clip_y1 - $y;
			$this->writer->write(sprintf('%.3F %.3F %.3F %.3F re %s', $x * Booknetic_Mpdf::SCALE, ($this->h - $y) * Booknetic_Mpdf::SCALE, $w * Booknetic_Mpdf::SCALE, -$ch * Booknetic_Mpdf::SCALE, $op));
			if (!empty($block_s)) {
				$tmp = "q\n" . sprintf('%.3F %.3F %.3F %.3F re %s', $x * Booknetic_Mpdf::SCALE, ($this->h - $y) * Booknetic_Mpdf::SCALE, $w * Booknetic_Mpdf::SCALE, -$ch * Booknetic_Mpdf::SCALE, $op);
				$tmp .= "\n" . $block_s . "\nQ";
				$block_s = $tmp;
			}
		}
		...
		foreach ($this->HTMLheaderPageLinks as $lk) {
			if ($rotate) {
				...
				$lx1 = (($lk[0] / Booknetic_Mpdf::SCALE));
				$ly1 = (($this->h - ($lk[1] / Booknetic_Mpdf::SCALE)));
				if ($rotate == 90) {
				...
				$lk[0] = $lx1 * Booknetic_Mpdf::SCALE;
				$lk[1] = ($this->h - $ly1) * Booknetic_Mpdf::SCALE;
			}
			if ($shrink_f != 1) {  // i.e. autofit has resized the box
				$lx1 = (($lk[0] / Booknetic_Mpdf::SCALE) - $x);
				$lx2 = $x + ($lx1 * $shrink_f);
				$lk[0] = $lx2 * Booknetic_Mpdf::SCALE;
				$ly1 = (($this->h - ($lk[1] / Booknetic_Mpdf::SCALE)) - $y);
				$ly2 = $y + ($ly1 * $shrink_f);
				$lk[1] = ($this->h - $ly2) * Booknetic_Mpdf::SCALE;
				$lk[2] *= $shrink_f; // width
	...
	function printbuffer($arrayaux, $blockstate = 0, $is_table = false, $table_draft = false, $cell_dir = '')
	{
		...
		for ($i = 0; $i < $array_size; $i++) {
			...
			if (($blockdir == 'rtl' || $this->biDirectional) && isset($arrayaux[$i][4]) && in_array($arrayaux[$i][4], ['ccourier', 'ctimes', 'chelvetica', 'csymbol', 'czapfdingbats'])) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException("You cannot use core fonts in a document which contains RTL text.");
			}
		}
		// mPDF 6
		...
		for ($i = 0; $i < $array_size; $i++) {
			...
			if (substr($vetor[0], 0, 3) == "\xbb\xa4\xac") { // identifier has been identified!
				...
				if ($objattr['type'] == 'nestedtable') {
					...
					list($skipln) = $this->inlineObject($objattr['type'], '', $this->y, $objattr, $this->lMargin, ($this->flowingBlockAttr['contentWidth'] / Booknetic_Mpdf::SCALE), $maxWidth, $this->flowingBlockAttr['height'], false, $is_table);
					...
					if ($objattr['type'] == 'image' && isset($objattr['float'])) {
						...
						$widthLeft = $maxWidth - ($this->flowingBlockAttr['contentWidth'] / Booknetic_Mpdf::SCALE);
	...
	function PaintDivBB($divider = '', $blockstate = 0, $blvl = 0)
	{
		...
		if ($this->blk[$blvl]['border_top'] && $divider != 'pagetop' && !$continuingpage) {
			...
			if (isset($tbd['s']) && $tbd['s']) {
				if (!$brset && $tbd['style'] != 'dotted' && $tbd['style'] != 'dashed') {
					$this->writer->write('q');
					$this->SetLineWidth(0);
					$this->writer->write(sprintf('%.3F %.3F m ', ($x0) * Booknetic_Mpdf::SCALE, ($this->h - ($y0)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $border_left) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $border_top)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w - $border_right) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $border_top)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w) * Booknetic_Mpdf::SCALE, ($this->h - ($y0)) * Booknetic_Mpdf::SCALE));
				...
				if ($brTR_H && $brTR_V) {
					...
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + $w) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + $w - ($border_top / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					}
				}
				/* -- BORDER-RADIUS -- */
				if ($brTL_H && $brTL_V) {
					if ($legend) {
						if ($legbreakR < ($x0 + $w - $brTR_H)) {
							$s .= (sprintf('%.3F %.3F l ', $legbreakR * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
						}
						if ($legbreakL > ($x0 + $brTL_H )) {
							$s .= (sprintf('%.3F %.3F m ', $legbreakL * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
							$s .= (sprintf('%.3F %.3F l ', ($x0 + $brTL_H ) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE) . "\n");
						} else {
							$s .= (sprintf('%.3F %.3F m ', ($x0 + $brTL_H ) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
						}
					} else {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $brTL_H ) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					}
					...
					if ($legend) {
						...
						if ($legbreakL > ($x0)) {
							$s .= (sprintf('%.3F %.3F m ', $legbreakL * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
							if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
								$s .= (sprintf('%.3F %.3F l ', ($x0) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
							} else {
								$s .= (sprintf('%.3F %.3F l ', ($x0 + ($border_top / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
							}
						} elseif ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
							$s .= (sprintf('%.3F %.3F m ', ($x0) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
						} else {
							$s .= (sprintf('%.3F %.3F m ', ($x0 + $border_top / 2) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
						}
					} elseif ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F l ', ($x0) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_top / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					} else {
		...
		if ($this->blk[$blvl]['border_bottom'] && $blockstate != 1 && $divider != 'pagebottom') {
			...
			if (isset($tbd['s']) && $tbd['s']) {
				if (!$brset && $tbd['style'] != 'dotted' && $tbd['style'] != 'dashed') {
					$this->writer->write('q');
					$this->SetLineWidth(0);
					$this->writer->write(sprintf('%.3F %.3F m ', ($x0) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $border_left) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - $border_bottom)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w - $border_right) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - $border_bottom)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(' h W n '); // Ends path no-op & Sets the clipping path
				}
				...
				if ($brBL_H && $brBL_V) {
					...
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F m ', ($x0) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - ($border_bottom / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + ($border_bottom / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - ($border_bottom / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					}
				}
				/* -- BORDER-RADIUS -- */
				if ($brBR_H && $brBR_V) {
					$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_bottom / 2) - $brBR_H ) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - ($border_bottom / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					$s .= ($this->_EllipseArc($x0 + $w - $brBR_H, $y0 + $h - $brBR_V, $brBR_H - $border_bottom / 2, $brBR_V - $border_bottom / 2, 4, 1)) . "\n";
				} else { 				/* -- END BORDER-RADIUS -- */
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $w) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - ($border_bottom / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_bottom / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - ($border_bottom / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					}
				}
				$s .= 'S' . "\n";
				$this->writer->write($s);
		...
		if ($this->blk[$blvl]['border_left']) {
			...
			if (isset($tbd['s']) && $tbd['s']) {
				if (!$brset && $tbd['style'] != 'dotted' && $tbd['style'] != 'dashed') {
					...
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $border_left) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $border_top)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $border_left) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - $border_bottom)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(' h W n '); // Ends path no-op & Sets the clipping path
				}
				...
				if ($brTL_V && $brTL_H) {
					...
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + ($border_left / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0)) * Booknetic_Mpdf::SCALE)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + ($border_left / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_left / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					}
				}
				/* -- BORDER-RADIUS -- */
				if ($brBL_V && $brBL_H) {
					$s .= (sprintf('%.3F %.3F l ', ($x0 + ($border_left / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - ($border_left / 2) - $brBL_V) ) * Booknetic_Mpdf::SCALE)) . "\n";
					$s .= ($this->_EllipseArc($x0 + $brBL_H, $y0 + $h - $brBL_V, $brBL_H - $border_left / 2, $brBL_V - $border_left / 2, 3, 1)) . "\n";
				} else { 				/* -- END BORDER-RADIUS -- */
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + ($border_left / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h) ) * Booknetic_Mpdf::SCALE)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + ($border_left / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - ($border_left / 2)) ) * Booknetic_Mpdf::SCALE)) . "\n";
					}
				}
				$s .= 'S' . "\n";
				$this->writer->write($s);
		...
		if ($this->blk[$blvl]['border_right']) {
			...
			if (isset($tbd['s']) && $tbd['s']) {
				if (!$brset && $tbd['style'] != 'dotted' && $tbd['style'] != 'dashed') {
					$this->writer->write('q');
					$this->SetLineWidth(0);
					$this->writer->write(sprintf('%.3F %.3F m ', ($x0 + $w) * Booknetic_Mpdf::SCALE, ($this->h - ($y0)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w - $border_right) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $border_top)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w - $border_right) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - $border_bottom)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(sprintf('%.3F %.3F l ', ($x0 + $w) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h)) * Booknetic_Mpdf::SCALE));
					$this->writer->write(' h W n '); // Ends path no-op & Sets the clipping path
				...
				if ($brBR_V && $brBR_H) {
					...
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + $w - ($border_right / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h)) * Booknetic_Mpdf::SCALE)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F m ', ($x0 + $w - ($border_right / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + $h - ($border_right / 2))) * Booknetic_Mpdf::SCALE)) . "\n";
					}
				}
				/* -- BORDER-RADIUS -- */
				if ($brTR_V && $brTR_H) {
					$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_right / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_right / 2) + $brTR_V) ) * Booknetic_Mpdf::SCALE)) . "\n";
					$s .= ($this->_EllipseArc($x0 + $w - $brTR_H, $y0 + $brTR_V, $brTR_H - $border_right / 2, $brTR_V - $border_right / 2, 1, 1)) . "\n";
				} else { 				/* -- END BORDER-RADIUS -- */
					if ($tbd['style'] == 'solid' || $tbd['style'] == 'double') {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_right / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0) ) * Booknetic_Mpdf::SCALE)) . "\n";
					} else {
						$s .= (sprintf('%.3F %.3F l ', ($x0 + $w - ($border_right / 2)) * Booknetic_Mpdf::SCALE, ($this->h - ($y0 + ($border_right / 2)) ) * Booknetic_Mpdf::SCALE)) . "\n";
					}
				}
				$s .= 'S' . "\n";
				$this->writer->write($s);
		...
		// Set clipping path
		$s = ' q 0 w '; // Line width=0
		$s .= sprintf('%.3F %.3F m ', ($bgx0 + $brbgTL_H ) * Booknetic_Mpdf::SCALE, ($this->h - $bgy0) * Booknetic_Mpdf::SCALE); // start point TL before the arc
		/* -- BORDER-RADIUS -- */
		if ($brbgTL_H || $brbgTL_V) {
			$s .= $this->_EllipseArc($bgx0 + $brbgTL_H, $bgy0 + $brbgTL_V, $brbgTL_H, $brbgTL_V, 2); // segment 2 TL
		}
		/* -- END BORDER-RADIUS -- */
		$s .= sprintf('%.3F %.3F l ', ($bgx0) * Booknetic_Mpdf::SCALE, ($this->h - ($bgy1 - $brbgBL_V )) * Booknetic_Mpdf::SCALE); // line to BL
		/* -- BORDER-RADIUS -- */
		if ($brbgBL_H || $brbgBL_V) {
			$s .= $this->_EllipseArc($bgx0 + $brbgBL_H, $bgy1 - $brbgBL_V, $brbgBL_H, $brbgBL_V, 3); // segment 3 BL
		}
		/* -- END BORDER-RADIUS -- */
		$s .= sprintf('%.3F %.3F l ', ($bgx1 - $brbgBR_H ) * Booknetic_Mpdf::SCALE, ($this->h - ($bgy1)) * Booknetic_Mpdf::SCALE); // line to BR
		/* -- BORDER-RADIUS -- */
		if ($brbgBR_H || $brbgBR_V) {
			$s .= $this->_EllipseArc($bgx1 - $brbgBR_H, $bgy1 - $brbgBR_V, $brbgBR_H, $brbgBR_V, 4); // segment 4 BR
		}
		/* -- END BORDER-RADIUS -- */
		$s .= sprintf('%.3F %.3F l ', ($bgx1) * Booknetic_Mpdf::SCALE, ($this->h - ($bgy0 + $brbgTR_V)) * Booknetic_Mpdf::SCALE); // line to TR
		/* -- BORDER-RADIUS -- */
		if ($brbgTR_H || $brbgTR_V) {
			$s .= $this->_EllipseArc($bgx1 - $brbgTR_H, $bgy0 + $brbgTR_V, $brbgTR_H, $brbgTR_V, 1); // segment 1 TR
		}
		/* -- END BORDER-RADIUS -- */
		$s .= sprintf('%.3F %.3F l ', ($bgx0 + $brbgTL_H ) * Booknetic_Mpdf::SCALE, ($this->h - $bgy0) * Booknetic_Mpdf::SCALE); // line to TL
		// Box Shadow
		$shadow = '';
		if (isset($this->blk[$blvl]['box_shadow']) && $this->blk[$blvl]['box_shadow'] && $h > 0) {
			foreach ($this->blk[$blvl]['box_shadow'] as $sh) {
				...
				$shadow .= sprintf('0 %.3F m %.3F %.3F l ', $this->h * Booknetic_Mpdf::SCALE, $this->w * Booknetic_Mpdf::SCALE, $this->h * Booknetic_Mpdf::SCALE);
				$shadow .= sprintf('%.3F 0 l 0 0 l 0 %.3F l ', $this->w * Booknetic_Mpdf::SCALE, $this->h * Booknetic_Mpdf::SCALE);
				$shadow .= 'W n' . "\n";
				...
				// Shadow Offset
				if ($sh['x'] || $sh['y']) {
					$shadow .= sprintf(' q 1 0 0 1 %.4F %.4F cm', $sh['x'] * Booknetic_Mpdf::SCALE, -$sh['y'] * Booknetic_Mpdf::SCALE) . "\n";
				}
				...
				$shadow .= sprintf('%.3F %.3F m ', ($p1x ) * Booknetic_Mpdf::SCALE, ($this->h - ($p1y )) * Booknetic_Mpdf::SCALE);
				$shadow .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', ($p1c2x) * Booknetic_Mpdf::SCALE, ($this->h - ($p1y)) * Booknetic_Mpdf::SCALE, ($p4x) * Booknetic_Mpdf::SCALE, ($this->h - ($p4c2y)) * Booknetic_Mpdf::SCALE, ($p4x) * Booknetic_Mpdf::SCALE, ($this->h - ($p4y)) * Booknetic_Mpdf::SCALE);
				$patch_array[0]['f'] = 0;
				...
				$shadow .= sprintf('%.3F %.3F l ', ($p3x ) * Booknetic_Mpdf::SCALE, ($this->h - ($p3y )) * Booknetic_Mpdf::SCALE);
				$patch_array[1]['f'] = 2;
				...
				$shadow .= sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c ', ($p1x) * Booknetic_Mpdf::SCALE, ($this->h - ($p1c2y)) * Booknetic_Mpdf::SCALE, ($p4c2x) * Booknetic_Mpdf::SCALE, ($this->h - ($p4y)) * Booknetic_Mpdf::SCALE, ($p4x) * Booknetic_Mpdf::SCALE, ($this->h - ($p4y)) * Booknetic_Mpdf::SCALE);
				$patch_array[2]['f'] = 2;
	...
	function _EllipseArc($x0, $y0, $rx, $ry, $seg = 1, $part = false, $start = false)
	{
		...
		$rx *= Booknetic_Mpdf::SCALE;
		$ry *= Booknetic_Mpdf::SCALE;
		...
		$dtm = $dt / 3;
		$x0 *= Booknetic_Mpdf::SCALE;
		$y0 = ($this->h - $y0) * Booknetic_Mpdf::SCALE;
		$t1 = $astart;
		$a0 = $x0 + ($rx * cos($t1));
		$b0 = $y0 + ($ry * sin($t1));
		$c0 = -$rx * sin($t1);
		...
		if (isset($arrayaux['FONT-SIZE'])) {
			...
			if (is_numeric($firstLetter) || ($firstLetter === '.')) {
				if ($type == 'BLOCK' && $this->blklvl > 0 && isset($this->blk[$this->blklvl - 1]['InlineProperties']) && isset($this->blk[$this->blklvl - 1]['InlineProperties']['size'])) {
					...
					$mmsize = $this->sizeConverter->convert($v, $this->default_font_size / Booknetic_Mpdf::SCALE);
				} else {
					$mmsize = $this->sizeConverter->convert($v, $this->FontSize);
				}
				$this->SetFontSize($mmsize * (Booknetic_Mpdf::SCALE), false); // Get size in points (pt)
			} else {
				$v = strtoupper($v);
	...
	// mPDF 6
		...
		if (($toonarrow) && ($this->table_error_report)) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException("Word is too long to fit in table - " . $this->table_error_report_param);
		}
		if ($toonarrow) {
			return -$biggestword;
		} else {
			...
			for ($j = 0; $j < $numcols; $j++) { // columns
				if (isset($cells[$i][$j]) && $cells[$i][$j]) {
					...
					if ($c['R'] && isset($c['textbuffer'])) {
						...
						if ($c['R'] >= 45 && $c['R'] < 90) {
							$tempch = ((sin(deg2rad($c['R']))) * $tempch ) + ((sin(deg2rad($c['R']))) * (($c['textbuffer'][0][11] / Booknetic_Mpdf::SCALE) / $this->shrin_k));
						}
						$this->SetFont($s_f, $s_st, $s_fs, true, true);
						$ch = ($tempch ) + $extrh;
					} else {
	...
	function _tableWrite(&$table, $split = false, $startrow = 0, $startcol = 0, $splitpg = 0, $rety = 0)
	{
		...
		if ($table['overflow'] == 'hidden' && $level == 1 && !$this->table_rotate && !$this->ColActive) {
			// Bounding rectangle to clip
			$this->tableClipPath = sprintf('q %.3F %.3F %.3F %.3F re W n', $x0 * Booknetic_Mpdf::SCALE, $this->h * Booknetic_Mpdf::SCALE, $this->blk[$this->blklvl]['inner_width'] * Booknetic_Mpdf::SCALE, -$this->h * Booknetic_Mpdf::SCALE);
			...
			foreach ($this->columnbuffer as $key => $s) {
				if (isset($s['rel_y'])) { // only process position sensitive data
					...
					if (isset($this->columnLinks[$s['col']][intval($s['x'])][intval($s['y'])])) {
						...
						$this->PageLinks[$this->page][$ref][0] += ($xadj * Booknetic_Mpdf::SCALE);
						$this->PageLinks[$this->page][$ref][1] -= ($yadj * Booknetic_Mpdf::SCALE);
				...
				foreach ($this->columnbuffer as $key => $s) {
					if (isset($s['rel_y'])) { // only process position sensitive data
						...
						if (($ratio > 1) && ($ratio <= $this->max_colH_correction)) {
							...
							if (isset($this->columnLinks[$s['col']][intval($s['x'])][intval($s['y'])])) {
								$ref = $this->columnLinks[$s['col']][intval($s['x'])][intval($s['y'])];
								$this->PageLinks[$this->page][$ref][1] -= ($yadj * Booknetic_Mpdf::SCALE); // y value
	...
	function columnAdjustPregReplace($type, $xadj, $yadj, $pattern, $subject)
	{
		...
		return str_replace($matches[0], $this->columnAdjustAdd($type, Booknetic_Mpdf::SCALE, $xadj, $yadj, $matches[1], $matches[2], $matches[3], $matches[4], $matches[5], $matches[6]), $subject);
	}
	/* -- END COLUMNS -- */
	...
	function printtablebuffer()
	{
		...
		foreach ($this->tbrot_Links as $p => $l) {
			...
			foreach ($l as $v) {
				...
				$w = $v[2] / Booknetic_Mpdf::SCALE;
				$h = $v[3] / Booknetic_Mpdf::SCALE;
				$ax = ($v[0] / Booknetic_Mpdf::SCALE) - $this->tbrot_x0;
				$ay = (($this->hPt - $v[1]) / Booknetic_Mpdf::SCALE) - $this->tbrot_y0;
				...
				$v[0] = $bx * Booknetic_Mpdf::SCALE;
				$v[1] = ($this->h - $by) * Booknetic_Mpdf::SCALE;
				$v[2] = $h * Booknetic_Mpdf::SCALE; // swap width and height
				$v[3] = $w * Booknetic_Mpdf::SCALE;
	...
	function Ellipse($x, $y, $rx, $ry, $style = 'S')
	{
		...
		$this->writer->write(sprintf('%.3F %.3F m %.3F %.3F %.3F %.3F %.3F %.3F c', ($x + $rx) * Booknetic_Mpdf::SCALE, ($h - $y) * Booknetic_Mpdf::SCALE, ($x + $rx) * Booknetic_Mpdf::SCALE, ($h - ($y - $ly)) * Booknetic_Mpdf::SCALE, ($x + $lx) * Booknetic_Mpdf::SCALE, ($h - ($y - $ry)) * Booknetic_Mpdf::SCALE, $x * Booknetic_Mpdf::SCALE, ($h - ($y - $ry)) * Booknetic_Mpdf::SCALE));
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c', ($x - $lx) * Booknetic_Mpdf::SCALE, ($h - ($y - $ry)) * Booknetic_Mpdf::SCALE, ($x - $rx) * Booknetic_Mpdf::SCALE, ($h - ($y - $ly)) * Booknetic_Mpdf::SCALE, ($x - $rx) * Booknetic_Mpdf::SCALE, ($h - $y) * Booknetic_Mpdf::SCALE));
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c', ($x - $rx) * Booknetic_Mpdf::SCALE, ($h - ($y + $ly)) * Booknetic_Mpdf::SCALE, ($x - $lx) * Booknetic_Mpdf::SCALE, ($h - ($y + $ry)) * Booknetic_Mpdf::SCALE, $x * Booknetic_Mpdf::SCALE, ($h - ($y + $ry)) * Booknetic_Mpdf::SCALE));
		$this->writer->write(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F c %s', ($x + $lx) * Booknetic_Mpdf::SCALE, ($h - ($y + $ry)) * Booknetic_Mpdf::SCALE, ($x + $rx) * Booknetic_Mpdf::SCALE, ($h - ($y + $ly)) * Booknetic_Mpdf::SCALE, ($x + $rx) * Booknetic_Mpdf::SCALE, ($h - $y) * Booknetic_Mpdf::SCALE, $op));
	}
		...
		if (!$this->is_utf8($html)) {
			...
			while (mb_convert_encoding(mb_convert_encoding($html, "UTF-32", "UTF-8"), "UTF-8", "UTF-32") != $html) {
				...
				if ($error && $error['message'] === 'iconv(): Detected an illegal character in input string') {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException('Invalid input characters. Did you set $mpdf->in_charset properly?');
				}
			...
			throw new \Booknetic_Mpdf\Booknetic_MpdfException("HTML contains invalid UTF-8 character(s). See log for further details");
		}
	...
	function purify_utf8_text($txt)
	{
		...
		if (!$this->is_utf8($txt)) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException("Text contains invalid UTF-8 character(s)");
		}
	...
	function WriteBarcode($code, $showtext = 1, $x = '', $y = '', $size = 1, $border = 0, $paddingL = 1, $paddingR = 1, $paddingT = 2, $paddingB = 2, $height = 1, $bgcol = false, $col = false, $btype = 'ISBN', $supplement = '0', $supplement_code = '', $k = 1)
	{
		...
		if ($arrcode === false) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Error in barcode string: ' . $codestr);
		}
		...
		if (($btype === 'EAN13' && $showtext) || $btype === 'ISBN' || $btype === 'ISSN') {
			...
			$loop = 0;
			while ($loop == 0) {
				$this->SetFontSize($codestr_fontsize * 1.4 * Booknetic_Mpdf::SCALE, false); // don't write
			...
			$this->SetFont('', '', $codestr_fontsize * 1.4 * Booknetic_Mpdf::SCALE, true, true); // * 1.4 because font height is only 7/10 of given mm
			// WORD SPACING
			if ($fbwi > $sz) {
				$xtra = $fbwi - $sz;
				$charspacing = $xtra / (strlen($codestr) - 1);
				if ($charspacing) {
					$this->writer->write(sprintf('BT %.3F Tc ET', $charspacing * Booknetic_Mpdf::SCALE));
				}
			}
		...
		$this->SetFontSize(($outerfontsize / 3) * 3 * $fh * $size * Booknetic_Mpdf::SCALE); // 3mm numerals (FontSize is larger to account for space above/below characters)
		...
		$this->SetFontSize(3 * $fh * $size * Booknetic_Mpdf::SCALE); // 3mm numerals (FontSize is larger to account for space above/below characters)
		...
		if ($supplement) { // EAN-2 or -5 Supplement
			...
			if ($supparrcode === false) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException('Error in barcode string (supplement): ' . $codestr . ' ' . $supplement_code);
			}

			if (strlen($supplement_code) != $supplement) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException('Barcode supplement incorrect: ' . $supplement_code);
			}
	...
	function transformScale($s_x, $s_y, $x = '', $y = '', $returnstring = false)
	{
		...
		if (($s_x == 0) or ( $s_y == 0)) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Please do not use values equal to zero for scaling');
		}

		$y = ($this->h - $y) * Booknetic_Mpdf::SCALE;
		$x *= Booknetic_Mpdf::SCALE;
	...
	function transformTranslate($t_x, $t_y, $returnstring = false)
	{
		...
		$tm[2] = 0;
		$tm[3] = 1;
		$tm[4] = $t_x * Booknetic_Mpdf::SCALE;
		$tm[5] = -$t_y * Booknetic_Mpdf::SCALE;
	...
	function AdjustHTML($html, $tabSpaces = 8)
	{
		...
		if (strlen($html) > $limit) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf(
	...
	function OverWrite($file_in, $search, $replacement, $dest = Destination::DOWNLOAD, $file_out = "mpdf")
	{
		...
		switch ($dest) {
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException('Unable to create output file: ' . $file_out);
				}

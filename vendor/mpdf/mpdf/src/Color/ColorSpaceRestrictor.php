...
class ColorSpaceRestrictor
{
	 ...
	 * @param \Booknetic_Mpdf\Color\ColorModeConverter $colorModeConverter
	 * @param int $mode
	 */
	public function __construct(Booknetic_Mpdf $mpdf, ColorModeConverter $colorModeConverter, $mode)
	{
		$this->mpdf = $mpdf;
	...
	private function restrictSpotColorSpace($c, &$PDFAXwarnings = [])
	{
		if (!isset($this->mpdf->spotColorIDs[$c[1]])) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException('Error: Spot colour has not been defined - ' . $this->mpdf->spotColorIDs[$c[1]]);
		}

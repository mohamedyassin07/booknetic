...
class CssManager
{
	...
	public function __construct(Booknetic_Mpdf $mpdf, Cache $cache, SizeConverter $sizeConverter, ColorConverter $colorConverter)
	{
		$this->mpdf = $mpdf;
		$this->cache = $cache;
		...
		foreach ($prop as $k => $v) {
			...
			if ($k === 'FONT') {
				...
				if (count($prop) == 2) {
					...
					if ($format) {
						$newprop['SHEET-SIZE'] = [$format[0] / Booknetic_Mpdf::SCALE, $format[1] / Booknetic_Mpdf::SCALE];
					}
	...
	private function getFileContents($path)
	{
		...
		if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
		}

...
class DirectWrite
{
	...
	public function __construct(Booknetic_Mpdf $mpdf, Otl $otl, SizeConverter $sizeConverter, ColorConverter $colorConverter)
	{
		$this->mpdf = $mpdf;
		$this->otl = $otl;
				...
				if ($l > $wmax) {
					...
					if ($sep == -1) {
						...
						if ($align === 'J') {
							...
							list($charspacing, $ws) = $this->mpdf->GetJspacing($nb_carac, $nb_spaces, (($w - 2) - $len_ligne) * Booknetic_Mpdf::SCALE, $inclCursive);
				...
				if ($l > $wmax) {
					...
					if ($sep == -1) {
						...
						if ($align === 'J') {
							...
							list($charspacing, $ws) = $this->mpdf->GetJspacing($nb_carac, $nb_spaces, (($w - 2) - $len_ligne) * Booknetic_Mpdf::SCALE, $false);
							$this->mpdf->SetSpacing($charspacing, $ws);
							//////////////////////////////////////////
						}
						$this->mpdf->Cell($w, $h, $tmp, 0, 2, $align, $fill, $link);
	...
	function CircularText($x, $y, $r, $text, $align = 'top', $fontfamily = '', $fontsizePt = 0, $fontstyle = '', $kerning = 120, $fontwidth = 100, $divider = '')
	{
		...
		if ($kerning == 0) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Please use values unequal to zero for kerning (CircularText)');
		}

		if ($fontwidth == 0) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Please use values unequal to zero for font width (CircularText)');
		}

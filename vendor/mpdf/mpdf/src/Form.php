...
class Form
{
	...
	public function __construct(Booknetic_Mpdf $mpdf, Otl $otl, ColorConverter $colorConverter, BaseWriter $writer, FormWriter $formWriter)
	{
		$this->mpdf = $mpdf;
		$this->otl = $otl;
	...
	function print_ob_textarea($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
	{
		...
		if ($this->mpdf->useActiveForms) {
			...
			$ClipPath = sprintf('q %.3F %.3F %.3F %.3F re W n ', $this->mpdf->x * Booknetic_Mpdf::SCALE, ($this->mpdf->h - $this->mpdf->y) * Booknetic_Mpdf::SCALE, $w * Booknetic_Mpdf::SCALE, -$h * Booknetic_Mpdf::SCALE);
	...
	function print_ob_imageinput($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir, $is_table)
	{
		...
		if ($this->mpdf->useActiveForms) {
			...
			$this->writer->write(sprintf('q %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q', $objattr['INNER-WIDTH'] * Booknetic_Mpdf::SCALE, $objattr['INNER-HEIGHT'] * Booknetic_Mpdf::SCALE, $objattr['INNER-X'] * Booknetic_Mpdf::SCALE, ($this->mpdf->h - ($objattr['INNER-Y'] + $objattr['INNER-HEIGHT'] )) * Booknetic_Mpdf::SCALE, $objattr['ID']));
		...
		if (isset($this->pdf_acro_array)) {
			...
			foreach ($this->form_fonts as $fn) {
				if (is_array($this->mpdf->fonts[$fn]['n'])) {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException('Cannot use fonts with SMP or SIP characters for interactive Form elements');
				}
	...
	function SetFormText($w, $h, $name, $value = '', $default = '', $title = '', $flags = [], $align = 'L', $hidden = false, $maxlen = -1, $js = '', $background_col = false, $border_col = false)
	{
		...
		if (!preg_match('/^[a-zA-Z0-9_:\-]+$/', $name)) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Field [' . $name . '] must have a name attribute, which can only contain letters, numbers, colon(:), undersore(_) or hyphen(-)');
		}
	...
	function SetFormButton($bb, $hh, $name, $value, $type, $title = '', $flags = [], $checked = false, $disabled = false, $background_col = false, $border_col = false, $noprint = false)
	{
		...
		if ($type === 'radio' || $type === 'checkbox') {
			if (!preg_match('/^[a-zA-Z0-9_:\-\.]+$/', $value)) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException("Field '" . $name . "' must have a value, which can only contain letters, numbers, colon(:), underscore(_), hyphen(-) or period(.)");
			}
		}
		if ($type === 'radio') {
	...
	function _form_rect($x, $y, $w, $h, $hPt)
	{
		...
		$y = $hPt - ($y * Booknetic_Mpdf::SCALE);
		$x2 = $x + ($w * Booknetic_Mpdf::SCALE);
		$y2 = $y - ($h * Booknetic_Mpdf::SCALE);
	...
	function _put_button_icon($array, $w, $h)
	{
		...
		if (!$info) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('Cannot find Button image');
		}

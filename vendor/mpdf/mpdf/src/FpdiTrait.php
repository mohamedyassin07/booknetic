...
trait FpdiTrait
{
	...
	protected $k = Booknetic_Mpdf::SCALE;
	...
	public function getImportedExternalPageLinks($pageNumber)
	{
		...
		if ($annotations instanceof PdfArray) {
			...
			foreach ($annotations->value as $annotation) {
				...
				$x1 = $getAttribute($position, 0) / Booknetic_Mpdf::SCALE;
				$y1 = $getAttribute($position, 1) / Booknetic_Mpdf::SCALE;
				$x2 = $getAttribute($position, 2) / Booknetic_Mpdf::SCALE;
				$y2 = $getAttribute($position, 3) / Booknetic_Mpdf::SCALE;
				$width = $x2 - $x1;
				$height = $y2 - $y1;

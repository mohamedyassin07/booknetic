...
class Gradient
{
	...
	public function __construct(Booknetic_Mpdf $mpdf, SizeConverter $sizeConverter, ColorConverter $colorConverter, BaseWriter $writer)
	{
		$this->mpdf = $mpdf;
	...
	{
		$s = ' q ';
		$s.=sprintf(' %.3F %.3F %.3F %.3F re W n ', $x * Booknetic_Mpdf::SCALE, ($this->mpdf->h - $y) * Booknetic_Mpdf::SCALE, $w * Booknetic_Mpdf::SCALE, -$h * Booknetic_Mpdf::SCALE);
		$s.=sprintf(' %.3F 0 0 %.3F %.3F %.3F cm ', $w * Booknetic_Mpdf::SCALE, $h * Booknetic_Mpdf::SCALE, $x * Booknetic_Mpdf::SCALE, ($this->mpdf->h - ($y + $h)) * Booknetic_Mpdf::SCALE);
	...
	public function Gradient($x, $y, $w, $h, $type, $stops = [], $colorspace = 'RGB', $coords = '', $extend = '', $return = false, $is_mask = false)
	{
		...
		$s = ' q';
		$s .= sprintf(' %.3F %.3F %.3F %.3F re W n', $x * Booknetic_Mpdf::SCALE, ($this->mpdf->h - $y) * Booknetic_Mpdf::SCALE, $w * Booknetic_Mpdf::SCALE, -$h * Booknetic_Mpdf::SCALE) . "\n";
		$s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm', $usew * Booknetic_Mpdf::SCALE, $useh * Booknetic_Mpdf::SCALE, $usex * Booknetic_Mpdf::SCALE, ($this->mpdf->h - ($usey + $useh)) * Booknetic_Mpdf::SCALE) . "\n";

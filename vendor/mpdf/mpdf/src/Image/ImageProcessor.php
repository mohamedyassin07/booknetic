...
namespace Booknetic_Mpdf\Image;
...
use Booknetic_Mpdf\File\StreamWrapperChecker;

use Booknetic_Mpdf\Gif\Gif;

use Booknetic_Mpdf\Language\LanguageToFontInterface;
use Booknetic_Mpdf\Language\ScriptToLanguageInterface;
...
use Booknetic_Mpdf\RemoteContentFetcher;
...
class ImageProcessor implements \Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \Booknetic_Mpdf\Image\ImageTypeGuesser
	 */
	private $guesser;
	...
	public function getImage(&$file, $firsttime = true, $allowvector = true, $orig_srcpath = false, $interpolation = false)
	{
		...
		if ($type === 'jpeg' || $type === 'jpg') {
			...
			if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3))) {
				...
				if (!function_exists('gd_info')) {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('JPG image may not use CMYK color space (%s).', $file));
				}
		...
		if ($type === 'png') {
			...
			if ($firsttime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
				...
				if ($pngalpha) {
					if ($this->mpdf->PDFA) {
						throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
					}
	...
	private function gzCompress($data)
	{
		if (!function_exists('gzcompress')) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('gzcompress is not available. install ext-zlib extension.');
		}
	...
	private function imageError($file, $firsttime, $msg)
	{
		...
		if ($firsttime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfImageException(sprintf('%s (%s)', $msg, substr($file, 0, 256)));
		}

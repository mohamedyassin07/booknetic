...
class SizeConverter implements \Psr\Log\LoggerAwareInterface
{
	...
	public function __construct($dpi, $defaultFontSize, Booknetic_Mpdf $mpdf, LoggerInterface $logger)
	{
		$this->dpi = $dpi;
	...
	public function convert($size = 5, $maxsize = 0, $fontsize = false, $usefontsize = true)
	{
		...
		switch ($unit) {
			...
			case 'pt':
				$size *= 1 / Booknetic_Mpdf::SCALE;
				break;

			case 'rem':
				$size *= $this->mpdf->default_font_size / Booknetic_Mpdf::SCALE;
				break;

...
use Booknetic_Mpdf\Conversion\DecToAlpha;
use Booknetic_Mpdf\Conversion\DecToRoman;
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ((isset($p['POSITION'])
			...
			if ($this->mpdf->inFixedPosBlock) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException('Cannot nest block with position:fixed or position:absolute');
			}
		...
		if ($currblk['inner_width'] < $mw) {
			...
			// if ($currblk['inner_width'] < $mw) { throw new \Booknetic_Mpdf\Booknetic_MpdfException("DIV is too narrow for text to fit!"); }
		}

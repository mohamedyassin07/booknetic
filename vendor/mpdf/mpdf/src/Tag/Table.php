...
class Table extends Tag
{
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if (isset($properties['FONT-SIZE'])) {
			if ($this->mpdf->tableLevel > 1) {
				...
				$mmsize = $this->sizeConverter->convert($properties['FONT-SIZE'], $this->mpdf->default_font_size / Booknetic_Mpdf::SCALE);
			}
			if ($mmsize) {
				$this->mpdf->default_font_size = $mmsize * Booknetic_Mpdf::SCALE;
	...
	public function close(&$ahtml, &$ihtml)
	{
		...
		if ($this->mpdf->tableLevel > 1) {
			...
			$this->mpdf->SetFont($this->mpdf->default_font, '', 0, false);
			$this->mpdf->default_font_size = $this->sizeConverter->convert($this->mpdf->base_table_properties['FONT-SIZE']) * Booknetic_Mpdf::SCALE;
			$this->mpdf->SetFontSize($this->mpdf->default_font_size, false);
		...
		if (isset($this->mpdf->table[1][1]['overflow']) && $this->mpdf->table[1][1]['overflow'] === 'visible') {
			if ($this->mpdf->kwt || $this->mpdf->table_rotate || $this->mpdf->table_keep_together || $this->mpdf->ColActive) {
				...
				$this->mpdf->table_rotate = 0;
				$this->mpdf->table_keep_together = false;
				//throw new \Booknetic_Mpdf\Booknetic_MpdfException("mPDF Warning: You cannot use CSS overflow:visible together with any of these functions:
		...
		if ($this->mpdf->table[1][1]['overflow'] === 'visible') {
			if ($maxrowheight > $fullpage) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException('mPDF Warning: A Table row is greater than available height. You cannot use CSS overflow:visible');
			}

		...
		for ($i = 1; $i <= $patternCount; $i++) {
			...
			if (false !== strpos($x_pos, '%')) {
				...
				if (isset($bpa['w']) && $bpa['w']) {
					$x_pos = ($bpa['w'] * $x_pos) - ($orig_w / Booknetic_Mpdf::SCALE * $x_pos);
				} else {
					$x_pos = ($w * $x_pos) - ($orig_w / Booknetic_Mpdf::SCALE * $x_pos);
				}
			}
			...
			if (false !== strpos($y_pos, '%')) {
				...
				if (isset($bpa['h']) && $bpa['h']) {
					$y_pos = ($bpa['h'] * $y_pos) - ($orig_h / Booknetic_Mpdf::SCALE * $y_pos);
				} else {
					$y_pos = ($h * $y_pos) - ($orig_h / Booknetic_Mpdf::SCALE * $y_pos);
				}
			}

			if (isset($bpa['x']) && $bpa['x']) {
				$adj_x = ($x_pos + $bpa['x']) * Booknetic_Mpdf::SCALE;
			} else {
				$adj_x = ($x_pos + $x) * Booknetic_Mpdf::SCALE;
			}

			if (isset($bpa['y']) && $bpa['y']) {
				$adj_y = (($pgh - $y_pos - $bpa['y']) * Booknetic_Mpdf::SCALE) - $orig_h;
			} else {
				$adj_y = (($pgh - $y_pos - $y) * Booknetic_Mpdf::SCALE) - $orig_h;
			}
			...
			if (!$img_obj) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException('Problem: Image object not found for background pattern ' . $img['i']);
			}

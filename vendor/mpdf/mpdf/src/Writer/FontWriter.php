...
class FontWriter
{
	...
	public function __construct(Booknetic_Mpdf $mpdf, BaseWriter $writer, FontCache $fontCache, $fontDescriptor)
	{
		$this->mpdf = $mpdf;
		$this->writer = $writer;
	...
	public function writeFonts()
	{
		...
		foreach ($this->mpdf->fonts as $k => $font) {
			...
			if ($type === 'Type0') {  // Adobe CJK Fonts
				...
				if ($this->mpdf->PDFA || $this->mpdf->PDFX) {
					throw new \Booknetic_Mpdf\Booknetic_MpdfException('Core fonts are not allowed in PDF/A1-b or PDFX/1-a files (Times, Helvetica, Courier etc.)');
				}
				...
				throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('Unsupported font type: %s (%s)', $type, $name));
			}
		}
	}

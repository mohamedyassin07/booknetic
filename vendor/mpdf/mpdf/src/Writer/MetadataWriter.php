...
use Booknetic_Mpdf\Utils\PdfDate;
...
class MetadataWriter implements \Psr\Log\LoggerAwareInterface
{
	...
	public function __construct(Booknetic_Mpdf $mpdf, BaseWriter $writer, Form $form, Protection $protection, LoggerInterface $logger)
	{
		$this->mpdf = $mpdf;
		$this->writer = $writer;
		$this->form = $form;
	...
	public function writeMetadata() // _putmetadata
	{
		...
		$Producer = 'mPDF' . ($this->mpdf->exposeVersion ? (' ' . Booknetic_Mpdf::VERSION) : '');
		$z = date('O'); // +0200
	  ...
	  <rdf:li>' . $this->mpdf->author . '</rdf:li>
		...
		elseif ($this->mpdf->PDFA) {
			...
			if (strpos($this->mpdf->PDFAversion, '-') === false) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('PDFA version (%s) is not valid. (Use: 1-B, 3-B, etc.)', $this->mpdf->PDFAversion));
			}
	...
	public function writeOutputIntent() // _putoutputintent
	{
		...
		if ($this->mpdf->ICCProfile) {
			if (!file_exists($this->mpdf->ICCProfile)) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('Unable to find ICC profile "%s"', $this->mpdf->ICCProfile));
			}
	...
	public function writeAssociatedFiles() // _putAssociatedFiles
	{
		if (!function_exists('gzcompress')) {
			throw new \Booknetic_Mpdf\Booknetic_MpdfException('ext-zlib is required for compression of associated files');
		}
		...
		foreach ($this->mpdf->associatedFiles as $k => $file) {
			...
			if (!$fileContent) {
				throw new \Booknetic_Mpdf\Booknetic_MpdfException(sprintf('Cannot access associated file - %s', $file['path']));
			}
	...
	public function writeAnnotations() // _putannots
	{
		...
		for ($n = 1; $n <= $nb; $n++) {
			...
			if (isset($this->mpdf->PageLinks[$n]) || isset($this->mpdf->PageAnnots[$n]) || count($this->form->forms) > 0) {
				...
				$wPt = $this->mpdf->pageDim[$n]['w'] * Booknetic_Mpdf::SCALE;
				$hPt = $this->mpdf->pageDim[$n]['h'] * Booknetic_Mpdf::SCALE;
				...
				if (isset($this->mpdf->PageLinks[$n])) {
					...
					foreach ($this->mpdf->PageLinks[$n] as $key => $pl) {
						...
						if (strpos($pl[4], '@') === 0) {
							...
							$htarg = $this->mpdf->pageDim[$p]['h'] * Booknetic_Mpdf::SCALE;
							...
							if (isset($this->mpdf->pageDim[$l[0]]['h'])) {
								$htarg = $this->mpdf->pageDim[$l[0]]['h'] * Booknetic_Mpdf::SCALE;
							} else {
								$htarg = $this->mpdf->h * Booknetic_Mpdf::SCALE;
							} // doesn't really matter

							$annot .= sprintf(' /Dest [%d 0 R /XYZ 0 %.3F null]>>', 1 + 2 * $l[0], $htarg - $l[1] * Booknetic_Mpdf::SCALE);
						}
				...
				if (isset($this->mpdf->PageAnnots[$n])) {
					...
					foreach ($this->mpdf->PageAnnots[$n] as $key => $pl) {
						...
						if ($this->mpdf->annotMargin != 0 || $x == 0 || $x < 0) { // Odd page, intentional non-strict comparison
							$x = ($wPt / Booknetic_Mpdf::SCALE) - $this->mpdf->annotMargin;
						}

						$w = $h = 0;
						$a = $x * Booknetic_Mpdf::SCALE;
						$b = $hPt - ($pl['y'] * Booknetic_Mpdf::SCALE);
						...
						if ($fileAttachment) {
							...
							if (!$file) {
								throw new \Booknetic_Mpdf\Booknetic_MpdfException('mPDF Error: Cannot access file attachment - ' . $pl['opt']['file']);
							}
							...
							if (is_array($pl['opt']['popup']) && isset($pl['opt']['popup'][0])) {
								$x = $pl['opt']['popup'][0] * Booknetic_Mpdf::SCALE;
							} else {
								$x = $pl['x'] * Booknetic_Mpdf::SCALE;
							}
							if (is_array($pl['opt']['popup']) && isset($pl['opt']['popup'][1])) {
								$y = $hPt - ($pl['opt']['popup'][1] * Booknetic_Mpdf::SCALE);
							} else {
								$y = $hPt - ($pl['y'] * Booknetic_Mpdf::SCALE);
							}
							if (is_array($pl['opt']['popup']) && isset($pl['opt']['popup'][2])) {
								$w = $pl['opt']['popup'][2] * Booknetic_Mpdf::SCALE;
							} else {
								$w = 180;
							}
							if (is_array($pl['opt']['popup']) && isset($pl['opt']['popup'][3])) {
								$h = $pl['opt']['popup'][3] * Booknetic_Mpdf::SCALE;
							} else {
								$h = 120;
							}

...
final class PageWriter
{
	...
	public function __construct(Booknetic_Mpdf $mpdf, Form $form, BaseWriter $writer, MetadataWriter $metadataWriter)
	{
		$this->mpdf = $mpdf;
		$this->form = $form;
		$this->writer = $writer;
	...
	public function writePages() // _putpages
	{
		...
		for ($n = 1; $n <= $nb; $n++) {
			...
			if (isset($this->mpdf->OrientationChanges[$n])) {
				$hPt = $this->mpdf->pageDim[$n]['w'] * Booknetic_Mpdf::SCALE;
				$wPt = $this->mpdf->pageDim[$n]['h'] * Booknetic_Mpdf::SCALE;
				$owidthPt_LR = $this->mpdf->pageDim[$n]['outer_width_TB'] * Booknetic_Mpdf::SCALE;
				$owidthPt_TB = $this->mpdf->pageDim[$n]['outer_width_LR'] * Booknetic_Mpdf::SCALE;
			} else {
				$wPt = $this->mpdf->pageDim[$n]['w'] * Booknetic_Mpdf::SCALE;
				$hPt = $this->mpdf->pageDim[$n]['h'] * Booknetic_Mpdf::SCALE;
				$owidthPt_LR = $this->mpdf->pageDim[$n]['outer_width_LR'] * Booknetic_Mpdf::SCALE;
				$owidthPt_TB = $this->mpdf->pageDim[$n]['outer_width_TB'] * Booknetic_Mpdf::SCALE;
			}
			...
			if (isset($this->mpdf->OrientationChanges[$n])) {
				...
				$bleedMargin = $this->mpdf->pageDim[$n]['bleedMargin'] * Booknetic_Mpdf::SCALE;

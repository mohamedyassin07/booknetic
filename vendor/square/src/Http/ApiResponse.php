...
class ApiResponse
{
     ...
     */
    public static function createFromContext($decodedBody, $result, HttpContext $context)
    {
        ...
        $cursor = isset( $decodedBody['cursor'] ) ? $decodedBody['cursor'] : null;
        $errors = [];
        ...
        if ($statusCode >= 400 && $statusCode < 600) {
            if (isset($decodedBody['errors'])) {
                ...
                $error = new \Square\Models\Error('V1_ERROR',  ( $decodedBody['type'] ) ? $decodedBody[ 'type' ] : 'Unknown');
                $error->setDetail( ( $decodedBody['message'] ) ? $decodedBody[ 'message' ] : null);
                $error->setField( ( $decodedBody['field'] ) ? $decodedBody[ 'field' ] : null);
                $errors = [$error];
            }
        }

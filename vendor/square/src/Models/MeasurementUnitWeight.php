<?php



namespace Square\Models;

/**
 * Unit of weight used to measure a quantity.
 */
class MeasurementUnitWeight
{
    /**
     * The weight is measured in ounces.
     */
    const IMPERIAL_WEIGHT_OUNCE = 'IMPERIAL_WEIGHT_OUNCE';

    /**
     * The weight is measured in pounds.
     */
    const IMPERIAL_POUND = 'IMPERIAL_POUND';

    /**
     * The weight is measured in stones.
     */
    const IMPERIAL_STONE = 'IMPERIAL_STONE';

    /**
     * The weight is measured in milligrams.
     */
    const METRIC_MILLIGRAM = 'METRIC_MILLIGRAM';

    /**
     * The weight is measured in grams.
     */
    const METRIC_GRAM = 'METRIC_GRAM';

    /**
     * The weight is measured in kilograms.
     */
    const METRIC_KILOGRAM = 'METRIC_KILOGRAM';
}
